...
// allowAuthorizer is the test-only ConsensusAuthorizer that lets every
// signed envelope through. The handshake e2e suite is about the PQ
// transport, not the authority predicate.
type allowAuthorizer struct{}

// Authorize always allows. Returns a single-reason allow so audit logs
// can still attribute the decision.
func (allowAuthorizer) Authorize(_ context.Context, _ Identity, _ string, _ Op) (Decision, error) {
	return Allow("test-allow-all"), nil
}

// newE2EIdentity builds a *keys.ServiceIdentity + envelope.IdentityHeader
// pair for the handshake tests. Uses the canonical abandon... about
// test mnemonic so derivation is deterministic.
func newE2EIdentity(t *testing.T, servicePath string) (*keys.ServiceIdentity, envelope.IdentityHeader) {
	t.Helper()
	ident, err := keys.NewServiceIdentity(testMnemonic, servicePath)
	if err != nil {
		t.Fatalf("NewServiceIdentity(%q): %v", servicePath, err)
	}
...
func bootServer(t *testing.T) (addr string, peerNodeID string) {
	...
	// Consensus-authorizer that allows any envelope through; the
	// handshake tests are about the PQ session, not the authority
	// predicate. allowAuthorizer is implemented at the bottom of this
	// file.
	srv := New(Config{
		Store:      secStore,
		MasterKey:  mk,
...
func TestHandshakeE2E_HybridSecretRoundTrip(t *testing.T) {
	...
	ident, hdr := newE2EIdentity(t, "ats/test-service")
	defer ident.Wipe()
...
func TestHandshakeE2E_FallbackClassicalOnly(t *testing.T) {
	...
	ident, hdr := newE2EIdentity(t, "ats/test-service-classical")
	defer ident.Wipe()
	c, err := zapclient.DialWithConfig(ctx, zapclient.Config{
...
func TestHandshakeE2E_SessionInstalled(t *testing.T) {
	...
	ident, hdr := newE2EIdentity(t, "ats/test-service-session")
	defer ident.Wipe()
	c, err := zapclient.DialWithConfig(ctx, zapclient.Config{
