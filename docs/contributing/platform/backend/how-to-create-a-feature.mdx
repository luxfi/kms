...
Suppose you're interested in implementing a new feature in KMS's backend, let's call it "feature-x." Here are the general steps you should follow.
...
Use the custom KMS function `ormify` in `src/lib/knex` for simple database operations within the DAL.
