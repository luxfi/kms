...
description: "This guide will help you contribute to the KMS SDK."
---

## Fork and clone the repo

[Fork](https://docs.github.com/en/get-started/quickstart/fork-a-repo) the [repository](https://github.com/KMS/sdk) to your own GitHub account and then [clone](https://docs.github.com/en/repositories/creating-and-managing-repositories/cloning-a-repository) it to your local device.
...
Start by creating a .env file at the root of the KMS directory then copy the contents of the file below into the .env file.
...
<Accordion title=".env file content">
  ```env 
    ...
    # https://lux.network/docs/documentation/platform/identities/universal-auth
    KMS_UNIVERSAL_CLIENT_ID=MACHINE_IDENTITY_CLIENT_ID
    KMS_UNIVERSAL_CLIENT_SECRET=MACHINE_IDENTITY_CLIENT_SECRET

    # The ID of the KMS project where we will create the test secrets.
    # NOTE: The project must have a dev environment. (This is created by default when you create a project.)
    KMS_PROJECT_ID=KMS_TEST_PROJECT_ID

    # The KMS site URL. If you are testing with a local KMS instance, then this should be set to "http://localhost:8080".
    KMS_SITE_URL=https://kms.lux.network
...
- **Target language**: The target language refers to the actual SDK code. For example, the [Node.js SDK](https://www.npmjs.com/package/@kms/sdk) is a "target language", and so is the [Python SDK](https://pypi.org/project/kms-python/).
...
fn main() -> Result<()> {
    ...
    write_schema_for!(kms_json::client::ClientSettings);
    // Input types for Client::run_command
    write_schema_for!(kms_json::command::Command);

    // Output types for Client::run_command
    // Only add structs which are direct results of SDK commands.
    write_schema_for_response! {
        kms::manager::secrets::GetSecretResponse,
        kms::manager::secrets::ListSecretsResponse,
        kms::manager::secrets::UpdateSecretResponse,
        kms::manager::secrets::DeleteSecretResponse,
        kms::manager::secrets::CreateSecretResponse, // <-- This is the output from the above example!
        kms::auth::AccessTokenSuccessResponse
    };
...
await ensureDir("./languages/node/src/kms_client");
writeToFile("./languages/node/src/kms_client/schemas.ts", ts.lines);
```
...
<Warning>
    After running the commands above, it's very important that you rename the generated .so file to `kms_py.so`. After renaming it you also need to move it into the root of the `crates/kms-py` folder.
</Warning>
...
After you created the function for creating the secret, you'll need need to add it to the ClientSecrets implementation. We do it this way to keep the code organized and easy to read. The ClientSecrets struct is located in the `crates/kms/src/manager/secrets.rs` file.
...
We define new commands in the `crates/kms-json/src/command.rs` file. The `Command` enum is what we use to define new commands.
...
After defining the command, we need to add it to the command handler itself. This takes place in the `crates/kms-json/src/client.rs` file. The `run_command` function is what we use to execute commands.
...
The build command will execute a build script in the `kms-napi` crate, and move the generated bindings to the appropriate folder.
...
export class KMSClient {
    ...
    constructor(settings: ClientSettings) {
        ...
        this.#client = new rust.KMSClient(settingsJson);
    }
...
The SDK has a lot of moving parts, and it can be a little overwhelming at first. But once you get the hang of it, it's actually quite simple. If you have any questions, feel free to reach out to us on [Slack](https://lux.network/slack), or [open an issue](https://github.com/KMS/sdk/issues) on GitHub.
