---
title: "FAQ"
description: "Frequently Asked Questions about contributing to KMS"
---

Frequently asked questions about contributing to KMS can be found on this page. 
...
<Accordion title="Error building KMS platform backend (Alpine Linux CDN temporary error)">
    ...
    for mirror in $ALPINE_MIRRORS; do \
        ...
        for i in $(seq $retries_left); do \
            ...
            if apk add --no-cache bash curl git && \
               curl -1sLf 'https://dl.cloudsmith.io/public/kms/kms-cli/setup.alpine.sh' | bash && \
               apk add --no-cache kms=0.8.1; then \
                break; \
            fi; \
            sleep 10; \
        done; \
