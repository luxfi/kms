...
description: "How to use KMS API to sync secrets to external secret managers"
---

The KMS API allows you to create programmatic integrations that connect with third-party secret managers to synchronize secrets from KMS.
...
<Steps>
  <Step title="Authenticate with AWS SSM">
    ...
    <ParamField body="workspaceId" type="string" required>
        The KMS project ID for the integration.
    </ParamField>
    ...
    ```bash Request
    curl --request POST \
        --url https://kms.lux.network/api/v1/integration-auth/access-token \
  ...
  <Step title="Configure the Synchronization Setup">
	Once authentication between AWS SSM and KMS is established, you can configure the synchronization behavior. 
	This involves specifying the source (environment and secret path in KMS) and the destination in SSM to which the secrets will be synchronized.
    ...
    <ParamField body="sourceEnvironment" type="string" required>
        The KMS environment slug from which secrets will be synchronized, e.g., `dev`.
    </ParamField>
    <ParamField body="secretPath" type="string" required>
        The KMS folder path from which secrets will be synchronized, e.g., `/some/path`. The root path is `/`.
    </ParamField>

    ```bash Request
    curl --request POST \
        --url https://kms.lux.network/api/v1/integration \
        --header 'Authorization: <authorization>' \
        --header 'Content-Type: application/json' \
        --data '{
...
<Check>
Congratulations! You have successfully set up an integration to synchronize secrets from KMS with AWS SSM.
