...
<Tabs>
  <Tab title="AWS Secrets manager">
    ...
    <ParamField body="sourceEnvironment" type="string" required>
				The KMS environment slug from where secrets will be synced from. Example: `dev`
		</ParamField>
    <ParamField body="secretPath" type="string" required>
				The KMS folder path from where secrets will be synced from. Example: `/some/path`. The root of the environment is `/`.
		</ParamField>
  </Tab>
