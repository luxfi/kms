...
description: "Learn how to migrate KMS from MongoDB to PostgreSQL."
---

This guide will provide step by step instructions on migrating your KMS instance running on MongoDB to the newly released PostgreSQL version of KMS.
The newly released Postgres version of KMS is the only version of KMS that will receive feature updates and patches going forward. 

<Tip>
  If you have a small set of secrets, we recommend you to download the secrets and upload them to your new instance of KMS instead of running the migration script.
</Tip>
...
<Steps>
  ...
  <Step title="Set Migration Mode">
    To prevent new data entries during the migration, set your KMS instance to migration mode by setting the environment variable `MIGRATION_MODE=true` and redeploying your instance. 
...
## Dump MongoDB
To speed up the data transformation process, the first step involves transferring the production data from KMS's MongoDB to a local machine. 
...
<Steps>
  <Step title="Clone KMS Repository">
    Clone the KMS MongoDB repository.
    ```
    git clone -b kms/v0.46.11-postgres https://github.com/luxfi/kms.git
    ```
  </Step>
  ...
  <Step title="Execute Migration Script">
    ...
    When executing the above command, you'll be asked to provide the MongoDB connection string for the database containing your production KMS data. Since our production Mongo data is transferred to a local Mongo instance, you should input the connection string for this local instance.
    ...
    ```
    postgres://kms:kms@localhost/kms?sslmode=disable
    ```
  </Step>
  ...
  <Step title="Store migration metadata">
    ...
    We highly recommend you to make a copy of this folder in case you need assistance from the KMS team during your migration process.  
...
At this stage, the data from the Mongo instance of KMS should have been successfully converted into its Postgres equivalent. 
...
<Steps>
  <Step title="Dump from local PostgreSQL">
   ```
   pg_dump -h localhost -U kms -Fc -b -v -f dumpfilelocation.sql -d kms
   ```
  </Step>
...
Once the data migration to PostgreSQL is complete, you're ready to deploy KMS using the deployment method of your choice.
For guidance on deployment options, please visit the [self-hosting documentation](/self-hosting/overview). 
Remember to transfer the necessary [environment variables](/self-hosting/configuration/envars) from the MongoDB version of KMS to the new Postgres based KMS; rest assured, they are fully compatible.

<Warning>
The first deployment of Postgres based KMS must be deployed with Docker image tag `v0.46.11-postgres`. 
After deploying this version, you can proceed to update to any subsequent versions.
</Warning>

## Additional discussion 
- When you visit KMS's [docker hub](https://hub.docker.com/r/kms/kms) page, you will notice that image tags end with `-postgres`. 
This is to indicate that this version of KMS runs on the new Postgres backend. Any image tag that does not end in `postgres` runs on MongoDB.