...
description: "Security hardening recommendations for production KMS deployments"
---

This document provides specific security hardening recommendations for production KMS deployments. These recommendations follow KMS's security model and focus on defense in depth.
...
These security configurations apply to **all** KMS deployments regardless of how you deploy.
...
- **Required ports**: KMS API (8080) and HTTPS (if applicable)
...
Set proper site URL for your KMS instance:
...
**Keep frequent upgrade cadence**. Regularly update to the latest KMS version for your deployment method.

## Deployment-Specific Hardening

### Docker Deployment

These recommendations are specific to Docker deployments of KMS.
...
**Note**: KMS requires temporary directory access for:
...
docker run --cap-drop=ALL kms/kms:latest
```
...
docker run kms/kms:v0.93.1-postgres
```
...
docker run --memory=1g --cpus=0.5 kms/kms:latest
```
...
**Regular updates**. Monitor [Docker Hub](https://hub.docker.com/r/kms/kms/tags) for new releases and update your image tags regularly.

### Kubernetes Deployment

These recommendations are specific to Kubernetes deployments of KMS.
...
spec:
  template:
    spec:
      ...
      containers:
        - name: kms
          image: kms/kms:v0.93.1-postgres
          securityContext:
...
**Regular updates**. Monitor [Docker Hub](https://hub.docker.com/r/kms/kms/tags) for new releases and update your deployment manifests with new image tags regularly.

### Linux Binary Deployment

These recommendations are specific to Linux binary deployments of KMS.

#### System User Management

**Create dedicated user account**. Run KMS under a dedicated service account:

```bash
# Create dedicated user
sudo useradd --system --shell /bin/false --home-dir /opt/kms kms

# Create application directory
sudo mkdir -p /opt/kms
sudo chown kms:kms /opt/kms
```
...
ExecStart=/opt/kms/kms-linux-amd64
Restart=always
RestartSec=10
...
sudo chmod 750 /etc/kms
sudo chown root:kms /etc/kms

# Create environment file
sudo touch /etc/kms/environment
sudo chmod 640 /etc/kms/environment
sudo chown root:kms /etc/kms/environment
```
...
sudo chmod 755 /opt/kms/kms-linux-amd64
sudo chown kms:kms /opt/kms/kms-linux-amd64
...
# Allow KMS API access
sudo ufw allow 8080/tcp

# Allow HTTPS (if terminating TLS at KMS)
...
**Regular updates**. Monitor [Cloudsmith releases](https://cloudsmith.io/~kms/repos/kms-core/packages) for new binary versions and update your installation regularly.
...
- **Root Key Protection**: HSM encrypts KMS's root encryption keys using hardware-protected keys
...
KMS can integrate with external KMS providers to encrypt project secrets, providing enterprise-grade key management:
...
- **Envelope Encryption**: KMS uses your cloud KMS to encrypt/decrypt project workspace keys, which in turn encrypt the actual secret data
- **Compliance**: Leverage your cloud provider's compliance certifications (FedRAMP, SOC2, ISO 27001)

#### Benefits for Production Deployments

- **Separation of Concerns**: Keys managed in your cloud infrastructure, separate from KMS
...
**Implement log monitoring**. Set up centralized logging for security analysis and audit trails. Configure your SIEM or logging platform to monitor KMS operations.

### Security Updates

**Regular security updates**. Monitor the [KMS repository](https://github.com/luxfi/kms) for security updates and apply them promptly.
...
**FIPS 140-3 Compliance**. KMS is actively working on FIPS 140-3 compliance to meet U.S. and Canadian government cryptographic standards. This will provide validated cryptographic modules for organizations requiring certified encryption implementations.
