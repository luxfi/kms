---
title: "Linux (HA)"
description: "KMS High Availability Deployment architecture for Linux"
---

This guide describes how to achieve a highly available deployment of KMS on Linux machines without containerization. The architecture provided serves as a foundation for minimum high availability, which you can scale based on your specific requirements.
...
| KMS Core           | 3     | 2 vCPU, 4 GB memory      |  n1-highcpu-2   | c5.large   |
...
| KMS Core 1     | 52.1.0.15  |
| KMS Core 2     | 52.1.0.16  |
| KMS Core 3     | 52.1.0.17  |
...
backend kms_backend
    ...
    server kms-3 52.1.0.17:8080 check inter 1s
```

### 5. Deploy KMS Core
<Tabs>
  <Tab title="Debian/Ubuntu">
    First, add the KMS repository:
    ```bash
    curl -1sLf \
    'https://dl.cloudsmith.io/public/kms/kms-core/setup.deb.sh' \
    | sudo -E bash
    ```
    ...
    <Info>
      For production environments, we strongly recommend installing a specific version of the package to maintain consistency across reinstalls. View available versions at [KMS Package Versions](https://cloudsmith.io/~kms/repos/kms-core/packages/).
    </Info>
  </Tab>
  ...
  <Tab title="RedHat/CentOS/Amazon Linux">
    ...
    ```bash
    curl -1sLf \
    'https://dl.cloudsmith.io/public/kms/kms-core/setup.rpm.sh' \
    | sudo -E bash
    ```
...
2. Monitor KMS logs: `sudo kms-ctl tail`
