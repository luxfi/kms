---
title: "Kubernetes (HA)"
description: "Reference architecture for self-hosting KMS on Kubernetes (HA)"
---
Deploying KMS on-premise with high availability requires expertise in networking, container orchestration, and database management. 
...
## Core Components
### Kubernetes Cluster
KMS is deployed on a Kubernetes cluster, which allows for container management, auto-scaling, and self-healing capabilities. 
...
This is the entry point where all other services will interact with KMS.
...
<Note>
KMS does not support Redis cluster mode, and since this is an active-passive setup, we use a simple Redis deployment with backup/restore for failover.
</Note>

#### PostgreSQL Backup and Restore
PostgreSQL is the single source of truth for nearly all application data on KMS.
...
KMS can be deployed across multiple data centers in an active-passive configuration for disaster recovery. In this setup, one data center serves as the active site while others remain as passive standbys.
...
- The active KMS deployment serving all user traffic
...
- A standby KMS deployment (not receiving traffic)
