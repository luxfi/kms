...
description: "Reference architecture for self-hosting KMS on Google Cloud Run."
---

## Overview
This guide outlines a reference architecture for deploying KMS in a self-hosted configuration using Google Cloud Run. 
...
- **Cloud Run:** KMS service is containerized and deployed as fully managed Cloud Run services.

- **Cloud SQL:** KMS uses Postgres as its persistence layer. As such, Cloud SQL for PostgreSQL is used.

- **MemoryStore for Redis:** To schedule jobs, process audit logs and cache  performance, KMS requires Redis.

## Securing KMS's root credential

- **Secrets Manager:** To secure KMS’s root credentials (database connection string, encryption key, etc.), 
we highly recommend that you use Google Secrets Manager and only allow the tasks running KMS to access them.
...
<Steps>
  ...
  **MemoryStore (Redis):**
  ...
  <Step title="Get the KMS Docker image">
    Visit [Docker Hub](https://hub.docker.com/r/kms/kms/tags) and select a version of KMS image you would like to deploy.
    Then, within Cloud Run, paste the URL of the specific KMS Docker image you would like to use within the `Container image URL` field.
  ...
  <Step title="Set the environment variables">
    For a minimal installation of KMS, you must configure the following environment variables:
  ...
  [View all available configurations](/self-hosting/configuration/envars).
    ...
    You will want to setup Postgres and Redis within Google Cloud Platform to connect to KMS.

    Once you have added the required environment variables to the `Environment Variables` section within Cloud Run, 
    create the container to get KMS up and running.
