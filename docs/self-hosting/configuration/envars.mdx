...
description: "Read how to configure environment variables for self-hosted KMS."
---

KMS accepts all configurations via environment variables. For a minimal self-hosted instance, at least `ENCRYPTION_KEY`, `AUTH_SECRET`, `DB_CONNECTION_URI` and `REDIS_URL` must be defined.
...
<ParamField query="HOST" type="string" default="localhost" optional>
  Specifies the network interface KMS will bind to when accepting incoming connections.

By default, KMS binds to `localhost`, which restricts access to connections from the same machine.
...
<ParamField query="TELEMETRY_ENABLED" type="string" default="true" optional>
  Telemetry helps us improve KMS but if you want to disable it you may set
  this to `false`.
</ParamField>
...
<ParamField
  ...
  optional
>
  Determines whether your KMS instance can automatically read the service
...
The following environment variables can be used to configure the KMS Rest API to allow or restrict access to resources from different origins.

<ParamField query="CORS_ALLOWED_ORIGINS" type="string" optional>

Specify a list of origins that are allowed to access the KMS API.
...
<Info>
  ...
  on the KMS database. This includes the ability to create new schemas,
...
Without email configuration, KMS's core functions like sign-up/login and secret operations work, but this disables multi-factor authentication, email invites for projects, alerts for suspicious logins, and all other email-dependent features.
...
<ParamField query="SMTP_USERNAME" type="string" default="none" optional>
  Credential to connect to host (e.g. team@lux.network)
</ParamField>
...
<ParamField query="SMTP_REQUIRE_TLS" type="bool" default="true" optional>
  If this is `true` and `SMTP_PORT` is not 465 then KMS tries to use
...
<ParamField
  ...
  optional
>
  If this is `true`, KMS will validate the server's SSL/TLS certificate
...
<ParamField query="SMTP_CUSTOM_CA_CERT" type="string" default="none" optional>
    If your SMTP server uses a certificate signed by a custom Certificate Authority, you should set this variable so that KMS can trust the custom CA.
    ...
    KMS highly encourages the following variables be used alongside this one for maximum security:
...
3. Set a name for your API Key, we recommend using "KMS," and select the "Restricted Key" option. You will need to enable the "Mail Send" permission as shown below:
...
```
  ...
  <Info>
    Remember that you will need to restart KMS for this to work properly.
  </Info>
</Accordion>
...
<Accordion title="AWS SES">
  <Steps>
    ...
    <Step title="Set up your SMTP environment variables">
      With your AWS SES SMTP credentials, you can now set up your SMTP environment variables for your KMS instance.
...
<Accordion title="Gmail">
  ...
  applications like KMS to authenticate with Gmail via your username and password.
...
To login into KMS with OAuth providers such as Google, configure the associated variables.

<ParamField query="DEFAULT_SAML_ORG_SLUG" type="string">

When set, all visits to the KMS login page will automatically redirect users of your KMS instance to the SAML identity provider associated with the specified organization slug.
...
<Accordion title="Okta SAML">
  Requires enterprise license. Please contact team@lux.network to get more
  information.
</Accordion>
...
You can configure third-party app connections for re-use across KMS Projects.
...
To help you sync secrets from KMS to services such as Github and Gitlab, KMS provides native integrations out of the box.
...
You can configure KMS to collect and expose telemetry data for analytics and monitoring.
