...
If you're working with Go Lang, the official [KMS Go SDK](https://github.com/kms/go-sdk) package is the easiest way to fetch and work with secrets for your application.

- [Package](https://pkg.go.dev/github.com/kms/go-sdk)
- [Github Repository](https://github.com/kms/go-sdk)
...
func main() {
	...
	client := kms.NewKMSClient(context.Background(), kms.Config{
		SiteUrl: "https://kms.lux.network", // Optional, default is https://kms.lux.network
...
This example demonstrates how to use the KMS Go SDK in a simple Go application. The application retrieves a secret named `API_KEY` from the `dev` environment of the `YOUR_PROJECT_ID` project.
...
```go
client := kms.NewKMSClient(context.Background(), kms.Config{
		SiteUrl: "https://kms.lux.network", // Optional, default is https://api.lux.network
	})
```
...
<ParamField query="options" type="object">
    <Expandable title="properties">
        <ParamField query="SiteUrl" type="string" optional default="https://kms.lux.network">
            The URL of the KMS API..
        </ParamField>
...
The KMS Go SDK supports automatic token refreshing. After using one of the auth methods such as Universal Auth, the SDK will automatically renew and re-authenticate when needed.
...
Retrieve a secret from KMS. By default `Secrets().Retrieve()` fetches and returns a shared secret.
...
Create multiple secrets in KMS.
...
Retrieve all within the KMS project and environment that client is connected to.
...
Update an existing folder in KMS.
...
List signing algorithms in KMS.
...
Encrypt data with a key in KMS KMS.
...
Decrypt data with a key in KMS KMS.
