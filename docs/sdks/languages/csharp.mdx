...
title: "KMS .NET SDK"
sidebarTitle: ".NET"
url: "https://github.com/luxfi/kms-dotnet-sdk?tab=readme-ov-file#kms-net-sdk"
icon: "/images/sdks/languages/dotnet.svg"
---
{/* 
If you're working with C#, the official [KMS C# SDK](https://github.com/KMS/sdk/tree/main/languages/csharp) package is the easiest way to fetch and work with secrets for your application.

- [Nuget Package](https://www.nuget.org/packages/KMS.Sdk)
- [Github Repository](https://github.com/KMS/sdk/tree/main/languages/csharp)
...
namespace Example
{
    class Program
    {
        static void Main(string[] args)
        {
          ...
          var kmsClient = new KMSClient(settings);
            ...
            var secret = kmsClient.GetSecret(getSecretOptions);
...
This example demonstrates how to use the KMS C# SDK in a C# application. The application retrieves a secret named `TEST` from the `dev` environment of the `PROJECT_ID` project.
...
namespace Example
{
    class Program
    {
        static void Main(string[] args)
        {
          ...
          var kmsClient = new KMSClient(settings); // <-- Your SDK client is now ready to use
        }
    }
}
```
...
<ParamField query="options" type="object">
    <Expandable title="properties">
        ...
        <ParamField query="SiteUrl" type="string" default="https://kms.lux.network" optional>
...
var secrets = kms.ListSecrets(options);
```
...
var secret = kms.GetSecret(options);
```
...
var newSecret = kms.CreateSecret(options);
```
...
var updatedSecret = kms.UpdateSecret(options);
```
...
var deletedSecret = kms.DeleteSecret(options);
```
...
```cs
var key = kms.CreateSymmetricKey();
```
...
var encryptedData = kms.EncryptSymmetric(options);
```
...
var decryptedPlaintext = kms.DecryptSymmetric(decryptOptions);
```
