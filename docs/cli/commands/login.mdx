---
title: "kms login"
description: "Login into KMS from the CLI"
---
...
If you have added multiple users, you can switch between the users by using the [user command](./user).
    ...
    <Info>
      When you authenticate with **any other method than `user`**, an access token will be printed to the console upon successful login. This token can be used to authenticate with the KMS API and the CLI by passing it in the `--token` flag when applicable.
...
<Accordion title="Universal Auth">
  ...
  <Steps>
    ...
    ```bash
      kms login --method=universal-auth --client-id=<client-id> --client-secret=<client-secret>
    ```
    </Step>
...
<Accordion title="Native Kubernetes">
  ...
  <Steps>
    ...
    ```bash
      ...
      kms login --method=kubernetes --machine-identity-id=<machine-identity-id> --service-account-token-path=<service-account-token-path>
    ```
    </Step>
  ...
  <Accordion title="Native Azure">
    ...
    <Steps>
      ...
      ```bash
        kms login --method=azure --machine-identity-id=<machine-identity-id>
      ```
      </Step>
    </Steps>
  ...
  <Accordion title="Native GCP ID Token">
    ...
    <Steps>
      ...
      ```bash
        kms login --method=gcp-id-token --machine-identity-id=<machine-identity-id>
      ```
      </Step>
    </Steps>
  ...
  <Accordion title="GCP IAM">
    ...
    <Steps>
      ...
      ```bash
        kms login --method=gcp-iam --machine-identity-id=<machine-identity-id> --service-account-key-file-path=<service-account-key-file-path>
      ```
      </Step>
    </Steps>
  ...
  <Accordion title="Native AWS IAM">
    ...
    <Steps>
      ...
      ```bash
        kms login --method=aws-iam --machine-identity-id=<machine-identity-id>
      ```
      </Step>
    </Steps>
  ...
  <Accordion title="OIDC Auth">
    ...
    <Steps>
      ...
      ```bash
        kms login --method=oidc-auth --machine-identity-id=<machine-identity-id> --jwt=<oidc-jwt>
      ```
      </Step>
    </Steps>
  </Accordion>
  ...
  <Accordion title="JWT Auth">
    ...
    <Steps>
      ...
      ```bash
        kms login --method=jwt-auth --jwt=<jwt> --machine-identity-id=<machine-identity-id>
      ```
      </Step>
    </Steps>
  </Accordion>
...
<AccordionGroup>
  <Accordion title="--method">
    ```bash
    kms login --method=<auth-method> # Optional, will default to 'user'.
    ```
  ...
  <Accordion title="--client-id">
    ```bash
    kms login --client-id=<client-id> # Optional, required if --method=universal-auth.
    ```
    ...
    <Tip>
      The `client-id` flag can be substituted with the `KMS_UNIVERSAL_AUTH_CLIENT_ID` environment variable.
    </Tip>

  </Accordion>
  <Accordion title="--client-secret">
    ```bash
    kms login --client-secret=<client-secret> # Optional, required if --method=universal-auth.
    ```
    #### Description
    The client secret of the universal auth machine identity. This is required if the `--method` flag is set to `universal-auth`.

    <Tip>
      The `client-secret` flag can be substituted with the `KMS_UNIVERSAL_AUTH_CLIENT_SECRET` environment variable.
    </Tip>

  </Accordion>
  <Accordion title="--machine-identity-id">
    ```bash
    kms login --machine-identity-id=<your-machine-identity-id> # Optional, required if --method=kubernetes, azure, gcp-id-token, gcp-iam, or aws-iam.
    ```
    ...
    <Tip>
      The `machine-identity-id` flag can be substituted with the `KMS_MACHINE_IDENTITY_ID` environment variable.
    </Tip>

  </Accordion>
  <Accordion title="--service-account-token-path">
    ```bash
    kms login --service-account-token-path=<service-account-token-path> # Optional Will default to '/var/run/secrets/kubernetes.io/serviceaccount/token'.
    ```
    ...
    <Tip>
    The `service-account-token-path` flag can be substituted with the `KMS_KUBERNETES_SERVICE_ACCOUNT_TOKEN_PATH` environment variable.
    </Tip>

  </Accordion>
  <Accordion title="--service-account-key-file-path">
    ```bash
    kms login --service-account-key-file-path=<gcp-service-account-key-file-path> # Optional, but required if --method=gcp-iam.
    ```
    ...
    <Tip>
      The `service-account-key-path` flag can be substituted with the `KMS_GCP_IAM_SERVICE_ACCOUNT_KEY_FILE_PATH` environment variable.
    </Tip>
  ...
  <Accordion title="--oidc-jwt">
    ```bash
    kms login --oidc-jwt=<oidc-jwt-token>
    ```
    ...
    <Tip>
    The `oidc-jwt` flag can be substituted with the `KMS_OIDC_AUTH_JWT` environment variable.
    </Tip>
...
In this example we'll be using the `universal-auth` method to login to obtain an KMS access token, which we will then use to fetch secrets with.

<Steps>
  <Step title="Obtain an access token">
        ```bash
          export KMS_TOKEN=$(kms login --method=universal-auth --client-id=<client-id> --client-secret=<client-secret> --silent --plain) # silent and plain is important to ensure only the token itself is printed, so we can easily set it as an environment variable.
        ```

        Now that we've set the `KMS_TOKEN` environment variable, we can use the CLI to interact with KMS. The CLI will automatically check for the presence of the `KMS_TOKEN` environment variable and use it for authentication.
        ...
        ```bash
          kms [command] --token=<your-access-token> # The token output from the login command.
        ```
    </Step>

      <Step title="Fetch all secrets from an evironment">
        ```bash
          kms secrets --projectId=<your-project-id --env=dev --recursive
        ```
