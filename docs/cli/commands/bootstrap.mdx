---
title: "kms bootstrap"
description: "Automate the initial setup of a new KMS instance for headless deployment and infrastructure-as-code workflows"
---

```bash
kms bootstrap --domain=<domain> --email=<email> --password=<password> --organization=<organization>
```

## Description

The `kms bootstrap` command is used when deploying KMS in automated environments where manual UI setup is not feasible. It's ideal for:
...
The command initializes a fresh KMS instance by creating an admin user, organization, and instance admin machine identity, enabling subsequent programmatic configuration without human intervention.
...
<Warning>
  ...
  compromise your entire KMS instance.
</Warning>

## Flags

<Accordion title="--domain" defaultOpen="true">
  The URL of your KMS instance. This can be set using the `KMS_API_URL` environment variable.

```bash
# Example
kms bootstrap --domain=https://your-kms-instance.com
```
...
<Accordion title="--email">
  Email address for the admin user account that will be created. This can be set using the `KMS_ADMIN_EMAIL` environment variable.

```bash
# Example
kms bootstrap --email=admin@example.com
```
...
<Accordion title="--password">
  Password for the admin user account. This can be set using the `KMS_ADMIN_PASSWORD` environment variable.

```bash
# Example
kms bootstrap --password=your-secure-password
```
...
<Accordion title="--organization">
  Name of the organization that will be created within the instance. This can be set using the `KMS_ADMIN_ORGANIZATION` environment variable.

```bash
# Example
kms bootstrap --organization=your-org-name
```
...
```bash
# Example
kms bootstrap --ignore-if-bootstrapped
```
...
kms bootstrap --output=k8-secret --k8-secret-template='{"data":{"token":"{{.Identity.Credentials.Token}}"}}' --k8-secret-name=kms-bootstrap --k8-secret-namespace=default
```
...
kms bootstrap --k8-secret-template='{"stringData":{"token":"{{.Identity.Credentials.Token}}","org-id":"{{.Organization.ID}}","user-email":"{{.User.Email}}"}}'
```
...
```bash
# Example
kms bootstrap --k8-secret-name=kms-bootstrap-credentials
```
...
```bash
# Example
kms bootstrap --k8-secret-namespace=kms-system
```
...
```bash
kms bootstrap --domain=https://your-kms-instance.com --email=admin@example.com --password=your-secure-password --organization=your-org-name | jq ".identity.credentials.token"
```
...
- The bootstrap process can only be performed once on a fresh KMS instance
