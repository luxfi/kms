...
<Tabs>
  <Tab title="Single command">
    ```bash
    kms run [options] -- [your application start command]

    # Example
    kms run [options] -- npm run dev
    ```
  ...
  <Tab title="Chained commands">
    ```bash
    kms run [options] --command [string command]

    # Example
    kms run [options] --command "npm run bootstrap && npm run dev start; other-bash-command"
    ```
...
Inject secrets from KMS into your application process.

## Subcommands & flags

<Accordion title="kms run" defaultOpen="true">
  Use this command to inject secrets into your applications process

```bash
$ kms run -- <your application command>
...
### Flags
  ...
  <Accordion title="--watch">
    By passing the `watch` flag, you are telling the CLI to watch for changes that happen in your KMS project.
    ...
    ```bash
      # Example
      kms run --watch -- printenv
    ```
  </Accordion>

  <Accordion title="--project-config-dir">
    Explicitly set the directory where the .kms.json resides. This is useful for some monorepo setups.

    ```bash
      # Example
      kms run --project-config-dir=/some-dir -- printenv
    ```
  </Accordion>
  ...
  <Accordion title="--command">
    ...
    ```bash
    # Example
    kms run --command="npm run build && npm run dev; more-commands..."
    ```
  ...
  <Accordion title="--projectId">
    ...
    ```bash
    # Example
    kms run --projectId=<project-id> -- npm run dev
    ```
  ...
  <Accordion title="--token">
    ...
    You may also expose the token to the CLI by setting the environment variable `KMS_TOKEN` before executing the run command. This will have the same effect as setting the token with `--token` flag
...
<Accordion title="--env">
  ...
  <Accordion title="--tags">
    ...
    ```bash
    # Example
    kms run --tags=tag1,tag2,tag3 -- npm run dev
    ```
  ...
  <Accordion title="--path">
    ...
    ```bash
    # Example
    kms run --path="/nextjs" -- npm run dev
    ```
