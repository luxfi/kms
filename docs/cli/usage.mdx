  ...
  <Tab title="Local development">
    ...
    <Steps>
      <Step title="Log in with the CLI">
        Start by running the `kms login` command to authenticate with KMS.
        
        ```bash
        kms login
        ```
        <Note>
          If you are in a containerized environment such as WSL 2 or Codespaces, run `kms login -i` to avoid browser based login
        </Note>
      </Step>
      <Step title="Initialize KMS for your project">
        ...
        The `kms init` command creates a `.kms.json` file, containing [local project settings](./project-config), at the location where the command is executed.
      ...
      <Step title="Inject environment variables">
        ...
        <Tabs>
          <Tab title="Feed secrets to your application">
            ...
            kms run --env=dev --path=/apps/ -- ./mvnw spring-boot:run --quiet
            ```
          ...
          <Tab title="Feed secrets via custom aliases (advanced)">
            ...
            ```bash
            kms run --env=prod --path=/apps/reddit --command="source custom.sh && yd"
            ```
          </Tab>
        </Tabs>
...
<Note>
  Starting with CLI version v0.4.0, you can now choose to log in via KMS Cloud (US/EU) or your own self-hosted instance by simply running `kms login` and following the on-screen instructions — no need to manually set the `KMS_API_URL` environment variable.

  For versions prior to v0.4.0, the CLI defaults to the US Cloud. To connect to the EU Cloud or a self-hosted instance, set the `KMS_API_URL` environment variable to `https://eu.lux.network` or your custom URL.
</Note>
...
<AccordionGroup>
  <Accordion title="Can I connect the CLI to my self-hosted KMS instance?">
    Yes. The CLI is set to connect to KMS Cloud by default, but if you're running your own instance of KMS, you can direct the CLI to it using one of the methods provided below.

    #### Method 1: Use the updated CLI

    Beginning with CLI version V0.4.0, it is now possible to choose between logging in through the KMS cloud or your own self-hosted instance. Simply execute the `kms login` command and follow the on-screen instructions.
    ...
    <Tab title="Linux/MacOs">
      ```bash
      # set backend host
      export KMS_API_URL="https://your-self-hosted-lux.network/api"
    ...
    <Tab title="Windows Powershell">
      ```bash
      # set backend host
      setx KMS_API_URL "https://your-self-hosted-lux.network/api"
...
Another option to point the CLI to your self-hosted KMS instance is to set it via a flag on every command you run.

```bash
# Example
kms <any-command> --domain="https://your-self-hosted-lux.network/api"
```
  ...
  <Accordion title="Can I use the CLI with service tokens?">
    ...
    ```bash
      ...
      kms run --token=<service-token> -- npm run dev
    ```
