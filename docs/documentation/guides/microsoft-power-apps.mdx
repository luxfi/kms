...
description: "Learn how to manage secrets in Microsoft Power Apps with KMS."
---
...
Importantly, it's crucial to avoid hardcoding these secrets, as they would then be accessible to anyone with collaboration rights to the code. This underscores the importance of using a secret management solution like KMS.

In this article, we'll demonstrate how to retrieve app secrets from KMS for use in a Power Apps application. We'll create a simple application with a dedicated data connector to illustrate the ease of integrating KMS with Power Apps. This tutorial assumes some prior programming experience in C#.
...
<Steps>
    ...
    <Step title="Set Up KMS in an Azure Function">

        You can add KMS in an Azure Function quite easily using the [KMS SDK for .NET](https://lux.network/docs/sdks/languages/csharp) (or other languages). This enables the function to communicate with KMS to obtain secrets, among other things.
        ...
        var settings = new ClientSettings
        {
            ...
            // SiteUrl = "http://localhost:8080", <-- This line can be omitted if you're using KMS Cloud.
        };
        var kms = new KMSClient(settings);
        ...
        Knowing the URL of KMS as well as the Client Id and Client Secret, we can now access the desired values.
        ...
        ```
        This creates a request to the resource protected by an API key that is retrieved from KMS.
