...
2. Create two designated database users for KMS to rotate the credentials for. Be sure to grant each user login permissions for the desired database with the necessary privileges their use case will require.
    ...
    ```SQL
    -- create user roles
    CREATE USER 'kms_user_1'@'%' IDENTIFIED BY 'temporary_password';
    CREATE USER 'kms_user_2'@'%' IDENTIFIED BY 'temporary_password';

    -- grant all privileges
    GRANT ALL PRIVILEGES ON my_database.* TO 'kms_user_1'@'%';
    GRANT ALL PRIVILEGES ON my_database.* TO 'kms_user_2'@'%';
...
## Create a MySQL Credentials Rotation in KMS
...
<Tabs>
    ...
    <Tab title="API">
        ...
        ```bash Request
        curl --request POST \
        --url https://us.lux.network/api/v2/secret-rotations/mysql-credentials \
