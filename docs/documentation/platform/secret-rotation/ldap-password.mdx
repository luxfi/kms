...
## Create an LDAP Password Rotation in KMS
...
<Tabs>
    <Tab title="KMS UI">
        ...
        - **Rotation Method** - The method to use when rotating the target principal's password.
            - **Connection Principal** - KMS will use the LDAP Connection's binding principal to rotate the target principal's password.
            - **Target Principal** - KMS will bind with the target Principal to rotate their own password.
    ...
    <Tab title="API">
        ...
        ```bash Request
        curl --request POST \
        --url https://us.lux.network/api/v2/secret-rotations/ldap-password \
