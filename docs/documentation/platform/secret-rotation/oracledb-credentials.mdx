...
2. Create two designated database users for KMS to rotate the credentials for. Be sure to grant each user login permissions for the desired database with the necessary privileges their use case will require.
    ...
    GRANT ALL PRIVILEGES TO KMS_USER_1;
    GRANT ALL PRIVILEGES TO KMS_USER_2;
    ```
...
## Create an Oracle Database Credentials Rotation in KMS
...
<Tabs>
    ...
    <Tab title="API">
        ...
        ```bash Request
        curl --request POST \
        --url https://us.lux.network/api/v2/secret-rotations/oracledb-credentials \
