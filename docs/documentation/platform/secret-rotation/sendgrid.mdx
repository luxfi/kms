...
Eliminate the use of long lived secrets by rotating Twilio SendGrid API keys with KMS.
...
Using the provided admin API key, KMS will attempt to create child API keys with the specified permissions. 
New keys will ge generated every time a rotation occurs. Behind the scenes, KMS uses the [SendGrid API](https://docs.sendgrid.com/api-reference/api-keys/create-api-keys) to generate new API keys.
