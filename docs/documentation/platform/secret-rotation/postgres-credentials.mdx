...
2. Create two designated database users for KMS to rotate the credentials for. Be sure to grant each user login permissions for the desired database with the necessary privileges their use case will require.
    ...
    ```SQL
    -- create user roles
    CREATE USER kms_user_1 WITH ENCRYPTED PASSWORD 'temporary_password';
    CREATE USER kms_user_2 WITH ENCRYPTED PASSWORD 'temporary_password';

    -- grant database connection permissions
    GRANT CONNECT ON DATABASE my_database TO kms_user_1;
    GRANT CONNECT ON DATABASE my_database TO kms_user_2;

    -- grant relevant table permissions
    GRANT ALL PRIVILEGES ON ALL TABLES IN SCHEMA public TO kms_user_1;
    GRANT ALL PRIVILEGES ON ALL TABLES IN SCHEMA public TO kms_user_2;
    ```
...
## Create a PostgreSQL Credentials Rotation in KMS
...
<Tabs>
    ...
    <Tab title="API">
        ...
        ```bash Request
        curl --request POST \
        --url https://us.lux.network/api/v2/secret-rotations/postgres-credentials \
