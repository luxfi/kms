...
The KMS MySQL secret rotation allows you to automatically rotate your MySQL database user's password at a predefined interval.
...
1. KMS connects to your database using the provided `admin` user account.
...
<AccordionGroup>
  ...
  <Accordion title="Why do you need root user account?">
	The admin account is used by KMS to update the credentials for `user-a` and `user-b`. 
