...
KMS's AWS IAM User secret rotation capability lets you update the **Access key** and **Secret access key** credentials of a target IAM user from within KMS
...
2. Configuring the rotation strategy in KMS with the credentials of the managing IAM user.
3. Pressing the **Rotate** button in the KMS dashboard to trigger the rotation of the target IAM user's credentials. The strategy can also be configured to rotate the credentials automatically at a specified interval.

In the following steps, we explore the end-to-end workflow for setting up this strategy in KMS.
...
<Steps>
	...
	<Step title="Configure the AWS IAM User secret rotation strategy in KMS">
		<Tabs>
			<Tab title="KMS UI">
			...
			<Tab title="API">
				...
				```bash Request
				curl --request POST \
					--url https://us.lux.network/api/v2/secret-rotations/aws-iam-user-secret \
