...
2. Create two designated database users for KMS to rotate the credentials for. Be sure to grant each user login permissions for the desired database with the necessary privileges their use case will require.

An example creation statement might look like:
    ```SQL
    -- create server-level logins
    CREATE LOGIN [kms_user_1] WITH PASSWORD = 'my-password';
    CREATE LOGIN [kms_user_2] WITH PASSWORD = 'my-password';
    GRANT CONNECT SQL TO [kms_user_1];
    GRANT CONNECT SQL TO [kms_user_2];

    -- create database-level users with login from above
    USE my_database;
    CREATE USER [kms_user_1] FOR LOGIN [kms_user_1];
    CREATE USER [kms_user_2] FOR LOGIN [kms_user_2];

    -- grant relevant permissions
    GRANT SELECT, INSERT, UPDATE, DELETE ON SCHEMA::dbo TO [kms_user_1];
    GRANT SELECT, INSERT, UPDATE, DELETE ON SCHEMA::dbo TO [kms_user_2];
    ```
...
## Create a Microsoft SQL Server Credentials Rotation in KMS
...
<Tabs>
    ...
    <Tab title="API">
        ...
        ```bash Request
        curl --request POST \
        --url https://us.lux.network/api/v2/secret-rotations/mssql-credentials \
