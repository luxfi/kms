...
Monitor and detect exposed secrets across your data sources, including code repositories, with KMS Secret Scanning.
...
All scan activities can be monitored in real-time through the KMS UI, which displays:
- Current scan status
...
If you're intentionally committing a test secret that the secret scanner might flag, you can instruct KMS to overlook that secret with the methods listed below.
...
You can then add the fingerprints of the findings you wish to exclude. The [KMS scan](/cli/scanning-overview) report provides a unique Fingerprint for each secret found.
