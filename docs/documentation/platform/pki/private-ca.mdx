...
description: "Learn how to create a Private CA hierarchy with KMS."
---
...
1. Configuring an KMS root CA with details like name, validity period, and path length — This step is optional if you wish to use an external root CA with KMS only serving the intermediate CAs.
...
<Steps>
    ...
    <Step title="Creating an intermediate CA">
        ...
        2.3a. If you created a root CA in step 1, select **KMS CA** for the **Parent CA Type** field.
...
<Steps>
  <Step title="Creating a root CA">
    ...
    ```bash Request
    curl --location --request POST 'https://kms.lux.network/api/v1/pki/ca' \
    ...
    By default, KMS creates a root CA with the `RSA_2048` key algorithm, validity period of 10 years, with no restrictions on path length;
  ...
  <Step title="Creating an intermediate CA">
    ...
    ```bash Request
    curl --location --request GET 'https://kms.lux.network/api/v1/pki/ca/<intermediate-ca-id>/csr' \
      --header 'Authorization: Bearer <access-token>' \
      --data-raw ''
    ```
    ...
    ```bash Request
    curl --location --request POST 'https://kms.lux.network/api/v1/pki/ca/<root-ca-id>/sign-intermediate' \
      --header 'Content-Type: application/json' \
      --data-raw '{
          "csr": "<csr>",
          "notAfter": "2029-06-12"
    ...
    ```bash Request
    curl --location --request POST 'https://kms.lux.network/api/v1/pki/ca/<intermediate-ca-id>/import-certificate' \
      --header 'Authorization: Bearer <access-token>' \
      --header 'Content-Type: application/json' \
      --data-raw '{
...
<Note>
  If renewing an intermediate CA chained to an KMS CA, then KMS will
...
<Tabs>
  ...
  To renew a CA certificate, make an API request to the [Renew CA](/api-reference/endpoints/certificate-authorities/renew) API endpoint, specifying the new `notAfter` date for the CA.
    ...
    ```bash Request
    curl --location --request POST 'https://kms.lux.network/api/v1/pki/ca/<ca-id>/renew' \
      --header 'Authorization: Bearer <access-token>' \
      --header 'Content-Type: application/json' \
      --data-raw '{
          "type": "existing",
...
<AccordionGroup>
  <Accordion title="What key algorithms are supported as part of private key generation and certificate signing?">
    KMS supports `RSA 2048`, `RSA 4096`, `ECDSA P-256`, `ECDSA P-384` key
    algorithms specified at the time of creating a CA.
  </Accordion>
  <Accordion title="Does KMS support CA renewal via new key pair">
    At the moment, KMS only supports CA renewal via same key pair. We
    anticipate supporting CA renewal via new key pair in the coming month.
  </Accordion>
  <Accordion title="Does KMS support chaining an Intermediate CA to an external CA?">
