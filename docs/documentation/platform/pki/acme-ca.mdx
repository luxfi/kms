...
description: "Learn how to automatically provision and manage TLS certificates using ACME Certificate Authorities like Let's Encrypt with KMS PKI"
---

## Concept

The KMS ACME integration allows you to connect with ACME (Automatic Certificate Management Environment) Certificate Authorities to automatically issue and manage publicly trusted TLS certificates for your [subscribers](/documentation/platform/pki/subscribers). This integration enables you to leverage established public CA infrastructure like Let's Encrypt while centralizing your certificate management within KMS.
...
```mermaid
graph TD
    A[ACME CA Provider<br>e.g., Let's Encrypt] <-->|ACME v2 Protocol| B[KMS]
...
As part of the workflow, you configure DNS provider credentials, register an ACME CA provider with KMS, and create subscribers to represent the certificates you wish to issue. Each issued certificate is automatically managed through its lifecycle, including renewal before expiration.
...
A typical workflow for using KMS with ACME Certificate Authorities consists of the following steps:

1. Setting up AWS Route53 or Cloudflare credentials with appropriate DNS permissions.
2. Creating an AWS/Cloudflare connection in KMS to store the credentials.
3. Registering an ACME Certificate Authority (like Let's Encrypt) with KMS.
4. Creating subscribers that use the ACME CA as their issuing authority.
5. Managing certificate lifecycle events such as issuance, renewal, and revocation through KMS.

## Understanding ACME DNS-01 Challenge

The DNS-01 challenge is the method used by ACME CA providers to verify that you control a domain before issuing a certificate. Here's how KMS handles this process:

1. **Challenge Request**: When you request a certificate, the ACME provider (like Let's Encrypt) issues a challenge token.

2. **DNS Record Creation**: KMS creates a TXT record at `_acme-challenge.<YOUR_DOMAIN>` with a value derived from the challenge token.
...
5. **Cleanup**: After validation completes successfully, KMS automatically removes the TXT record from your DNS.
...
In the following steps, we explore how to set up ACME Certificate Authority integration with KMS using Let's Encrypt as an example.
...
<Steps>
    ...
    <Step title="Register ACME Certificate Authority">
        <Tabs>
          <Tab title="KMS UI">
            <Steps>
                <Step title="Create ACME CA">
                    ...
                    - **DNS App Connection**: Select from available DNS app connections or configure a new one. This connection provides KMS with the credentials needed to create and remove DNS records for ACME validation.
                    ...
                    Finally, press **Create** to register the ACME CA with KMS.
                </Step>
          ...
          <Tab title="API">
            To register an ACME CA with KMS using the API, make a request to the Create External CA endpoint:

            ### Sample request

            ```bash Request
            curl 'https://kms.lux.network/api/v1/pki/ca/acme' \
    ...
    <Step title="Issue Certificate">
        Once your subscriber is configured, you can issue certificates either through the KMS UI or programmatically via the API.

        When you request a certificate:
        1. KMS generates a key pair for the certificate
        ...
        7. KMS stores and manages the certificate for your subscriber
    ...
    <Step title="Use Certificate in Your Applications">
        The issued certificate and private key are now available through KMS and can be:

        - Downloaded directly from the KMS UI
        - Retrieved via the KMS API for programmatic access using the [latest certificate bundle endpoint](/api-reference/endpoints/pki/subscribers/get-latest-cert-bundle)
    </Step>
</Steps>

## Example: Let's Encrypt Integration

Let's Encrypt is a free, automated, and open Certificate Authority that provides domain-validated SSL/TLS certificates. Here's how the integration works with KMS:
...
<AccordionGroup>
  <Accordion title="What DNS validation methods are supported?">
    Currently, KMS supports DNS-01 validation through AWS Route53 or Cloudflare. The DNS-01 challenge method is preferred for ACME integrations because it:
  ...
  <Accordion title="How long are ACME certificates valid?">
    ...
    When configured, KMS automatically handles certificate renewal for subscribers.
  </Accordion>
