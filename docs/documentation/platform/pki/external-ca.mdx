...
description: "Learn how to connect External Certificate Authorities with KMS."
---

## Concept

In addition to creating a Private CA hierarchy, KMS allows you to integrate with External Certificate Authorities (CAs) to issue digital certificates for your [subscribers](/documentation/platform/pki/subscribers). This integration enables you to leverage established certificate authority infrastructure while centralizing your certificate management within KMS.

<div align="center">

```mermaid
graph TD
    B[KMS] -->|Manages Certificates| D[Subscribers]
...
When you integrate an External CA with KMS, you benefit from:
...
3. **Automation**: Leverage KMS's automation capabilities for certificate lifecycle management.
...
A typical workflow for integrating an External CA with KMS consists of the following steps:
...
3. **Register External CA**: Add the External CA configuration to your KMS project.
4. **Create Subscribers**: Set up subscribers that use the External CA as their issuing authority.
5. **Manage Certificate Lifecycle**: Handle certificate issuance, renewal, and revocation through KMS's unified interface.

The specific steps and requirements vary depending on the External CA type you choose to integrate.

## Supported Integration Methods

KMS currently supports integration with External Certificate Authorities through the following protocol:

### ACME Protocol Integration

ACME (Automatic Certificate Management Environment) is a widely adopted protocol for automated certificate issuance and management. KMS can integrate with any CA that supports the ACME protocol, including:
...
Managing External CAs through KMS provides several advantages over direct CA management:
...
<AccordionGroup>
  <Accordion title="Which External CAs does KMS currently support?">
    Currently, KMS supports any Certificate Authority that implements the ACME protocol, including:
  ...
  <Accordion title="How does certificate renewal work with External CAs?">
    Certificate reissuance is handled automatically by KMS based on the CA type:
    ...
    All certificate lifecycle events are tracked and managed through KMS's unified interface, ensuring continuous certificate validity.
  </Accordion>
  <Accordion title="What authentication methods are supported for External CAs?">
    ...
    KMS securely stores and manages all authentication credentials.
  </Accordion>
  <Accordion title="Can I enforce policies on certificates from External CAs?">
    Yes, KMS provides policy enforcement capabilities:
