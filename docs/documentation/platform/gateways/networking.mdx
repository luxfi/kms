---
title: "Networking"
description: "Network configuration and firewall requirements for KMS Gateway"
---

The KMS Gateway requires outbound network connectivity to establish secure communication with KMS's relay infrastructure. 
...
2. **Relay Servers** facilitate secure communication between Gateway and KMS Cloud
...
| TCP | kms.lux.network / eu.lux.network | 443 | API communication and relay allocation |

### Relay Server IP Addresses

Your firewall must allow outbound connectivity to the following KMS relay servers on dynamically allocated ports.

<Tabs>
  <Tab title="KMS cloud (US)">
    ```
  ...
  <Tab title="KMS dedicated">
    Please contact your KMS account manager for dedicated relay server IP addresses.
  </Tab>
</Tabs>
...
- **Outbound HTTPS** to kms.lux.network/eu.lux.network on port 443
...
- **Outbound HTTPS** to KMS API endpoints
...
- **No manual selection**: The KMS API automatically assigns the optimal relay server based on availability and proximity
...
- **End-to-end encryption**: All traffic between the gateway and KMS Cloud is encrypted using mutual TLS with certificate pinning
