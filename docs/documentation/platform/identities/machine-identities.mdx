...
description: "Learn how to use Machine Identities to programmatically interact with KMS."
---

## Concept

An KMS machine identity is an entity that represents a workload or application that require access to various resources in KMS. This is conceptually similar to an IAM user in AWS or service account in Google Cloud Platform (GCP).

Each identity must authenticate with the KMS API using a supported authentication method like [Token Auth](/documentation/platform/identities/token-auth), [Universal Auth](/documentation/platform/identities/universal-auth), [Kubernetes Auth](/documentation/platform/identities/kubernetes-auth), [AWS Auth](/documentation/platform/identities/aws-auth), [Azure Auth](/documentation/platform/identities/azure-auth), or [GCP Auth](/documentation/platform/identities/gcp-auth) to get back a short-lived access token to be used in subsequent requests.
...
- Auth/Token Configuration: Identities must be configured with corresponding authentication methods and access token properties to securely interact with the KMS API.
...
3. Authenticating the identity with the KMS API based on the configured authentication method on it and receiving a short-lived access token back.
4. Authenticating subsequent requests with the KMS API using the short-lived access token.

## Authentication Methods

To interact with various resources in KMS, Machine Identities can authenticate with the KMS API using:

- [Token Auth](/documentation/platform/identities/token-auth): A platform-agnostic, simple authentication method suitable to authenticate with KMS using a token.
- [Universal Auth](/documentation/platform/identities/universal-auth): A platform-agnostic authentication method suitable to authenticate with KMS using a Client ID and Client Secret.
...
<Accordion title="What is the difference between an identity and service token?">
  A service token is a project-level authentication method that is being deprecated in favor of identities. The service token method will be removed in the future in accordance with the deprecation notice and timeline stated [here](https://lux.network/blog/deprecating-api-keys).
  
  Amongst many differences, identities provide broader access over the KMS API, utilizes the same
