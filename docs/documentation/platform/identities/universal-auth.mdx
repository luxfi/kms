...
description: "Learn how to authenticate to KMS from any platform or environment."
---
...
The following sequence diagram illustrates the Universal Auth workflow for authenticating clients with KMS.
...
sequenceDiagram
  ...
  Note over Client,Infis: Step 3: Access KMS API with Token
  Client->>Infis: Make authenticated requests using the short-lived access token
```

## Concept

In this method, KMS authenticates a client by verifying the credentials issued for it at the `/api/v1/auth/universal-auth/login` endpoint. If successful,
then KMS returns a short-lived access token that can be used to make authenticated requests to the KMS API.

To be more specific:

1. The client submits a **Client ID** and **Client Secret** to KMS at the `/api/v1/auth/universal-auth/login` endpoint.
2. KMS verifies the credential pair.
3. If all is well, KMS returns a short-lived access token that the client can use to make authenticated requests to the KMS API.

## Guide

In the following steps, we explore how to create and use identities for your workloads and applications to access the KMS API
...
<Steps>
  <Step title="Creating an identity">
    ...
    If you're using KMS Cloud, then it is available under the Pro Tier. If you're self-hosting KMS, then you should contact sales@lux.network to purchase an enterprise license to use it.
    </Warning>
  ...
  <Step title="Creating a Client Secret">
    ...
    and password used to authenticate with the KMS API.
  ...
  <Step title="Accessing the KMS API with the identity">
    To access the KMS API as the identity, you should first perform a login operation
    ...
    <Tip>
      ...
      - For KMS Cloud US users: `https://kms.lux.network`
      - For KMS Cloud EU users: `https://eu.lux.network`
    </Tip>

    #### Sample request

    ```bash Request
    curl --location --request POST 'https://kms.lux.network/api/v1/auth/universal-auth/login' \
    ...
    Next, you can use the access token to authenticate with the [KMS API](/api-reference/overview/introduction)
...
4. The workload authenticates with KMS using the Client Secret and Client ID to obtain the initial access token (JWT).
5. The workload uses the access token to authenticate and continuously renews it before expiration:

```bash
curl --location --request POST 'https://kms.lux.network/api/v1/auth/universal-auth/renew' \
  --header 'Authorization: Bearer <accessToken>'
```

This approach allows your workload to securely bootstrap and maintain access to KMS without hard-coded secrets, solving the secret zero problem.

**FAQ**

<AccordionGroup>
  <Accordion title="Why is the KMS API rejecting my identity credentials?">
