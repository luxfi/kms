---
title: CircleCI
description: "Learn how to authenticate CircleCI jobs with KMS using OpenID Connect (OIDC)."
---
...
The following sequence diagram illustrates the OIDC Auth workflow for authenticating CircleCI jobs with KMS.
...
2. The CircleCI OIDC token is sent to KMS at the `/api/v1/auth/oidc-auth/login` endpoint.
3. KMS fetches the public key that was used to sign the identity token provided by CircleCI.
...
5. If all is well, KMS returns a short-lived access token that CircleCI jobs can use to make authenticated requests to the KMS API.

<Note>KMS needs network-level access to the CircleCI servers.</Note>
...
<Steps>
    ...
    <Step title="Using CircleCI OIDC token to authenticate with KMS">
        The following is an example of how to use the `$CIRCLE_OIDC_TOKEN` with the KMS [terraform provider](https://registry.terraform.io/providers/KMS/kms/latest/docs) to manage resources in a CircleCI pipeline.
        ...
        terraform-apply:
            ...
            - run:
                command: |
                    export KMS_AUTH_JWT="$CIRCLE_OIDC_TOKEN"
                    terraform init
        ...
        build-and-test:
            ...
            - terraform-apply
        ```
        The KMS terraform provider expects the  `KMS_AUTH_JWT` environment variable to be set to the CircleCI OIDC token.
        ```hcl main.tf
        terraform {
            required_providers {
                kms = {
        ...
        resource "kms_access_approval_policy" "prod-access-approval" {
            ...
            {
                type = "user"
                username = "sheen+200@lux.network"
            },
            ]
