---
title: Github
description: "Learn how to authenticate Github workflows with KMS using OpenID Connect (OIDC)."
---
...
The following sequence diagram illustrates the OIDC Auth workflow for authenticating Github workflows with KMS.
...
3. KMS fetches the public key that was used to sign the identity token from Github's identity provider using OIDC Discovery.
...
5. If all is well, KMS returns a short-lived access token that the Github workflow can use to make authenticated requests to the KMS API.

<Note>
  KMS needs network-level access to Github's identity provider endpoints.
</Note>
...
<Steps>
    ...
    <Step title="Accessing the KMS API with the identity">
        ...
        To access the KMS API as the identity, you need to fetch an identity token from Github's identity provider and make a request to the `/api/v1/auth/oidc-auth/login` endpoint in exchange for an access token.
        ...
        Below is an example of how a Github workflow can be configured to fetch secrets from KMS using the [KMS Secrets Action](https://github.com/KMS/secrets-action) with OIDC Auth.
        ```yaml
        ...
        jobs:
            build:
                ...
                steps:
                    - uses: KMS/secrets-action@v1.0.7
                    with:
                        method: "oidc"
                        env-slug: "dev"
        ...
        <Tip>
            We recommend using [KMS Secrets Action](https://github.com/KMS/secrets-action) to authenticate with KMS using OIDC Auth as it handles the authentication process including the fetching of identity tokens for you.
        </Tip>
