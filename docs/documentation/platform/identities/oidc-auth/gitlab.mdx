---
title: GitLab
description: "Learn how to authenticate GitLab pipelines with KMS using OpenID Connect (OIDC)."
---
...
The following sequence diagram illustrates the OIDC Auth workflow for authenticating GitLab pipelines with KMS.
...
At a high-level, KMS authenticates a client by verifying the JWT and checking that it meets specific requirements (e.g. it is issued by a trusted identity provider) at the `/api/v1/auth/oidc-auth/login` endpoint. If successful,
...
2. The fetched identity token is sent to KMS at the `/api/v1/auth/oidc-auth/login` endpoint.
3. KMS fetches the public key that was used to sign the identity token from GitLab's identity provider using OIDC Discovery.
4. KMS validates the JWT using the public key provided by the identity provider and checks that the subject, audience, and claims of the token matches with the set criteria.
5. If all is well, KMS returns a short-lived access token that the GitLab pipeline can use to make authenticated requests to the KMS API.

<Note>
  KMS needs network-level access to GitLab's identity provider endpoints.
</Note>

## Guide

In the following steps, we explore how to create and use identities to access the KMS API using the OIDC Auth authentication method.
...
<Steps>
    ...
    <Step title="Accessing the KMS API with the identity">
        As demonstration, we will be using the KMS CLI to fetch KMS secrets and utilize them within a GitLab pipeline.

        To access KMS secrets as the identity, you need to use an identity token from GitLab which matches the OIDC configuration defined for the machine identity.
        This can be done by defining the `id_tokens` property. The resulting token would then be used to login with OIDC like the following: `kms login --method=oidc-auth --oidc-jwt=$GITLAB_TOKEN`

        Below is a complete example of how a GitLab pipeline can be configured to work with secrets from KMS using the KMS CLI with OIDC Auth:
        ...
        script:
            ...
            - export KMS_TOKEN=$(kms login --method=oidc-auth --machine-identity-id=4e807a78-1b1c-4bd6-9609-ef2b0cf4fd54 --oidc-jwt=$KMS_ID_TOKEN --silent --plain)
            - kms run --projectId=1d0443c1-cd43-4b3a-91a3-9d5f81254a89 --env=dev -- npm run build
        ```

        The `id_tokens` keyword is used to request an ID token for the job. In this example, an ID token named `KMS_ID_TOKEN` is requested with the audience (`aud`) claim set to "kms-aud-test". This ID token will be used to authenticate with KMS.
        <Note>
        ...
        If an identity access token expires, it can no longer authenticate with the KMS API. In this case, a new access token should be obtained by performing another login operation.
        </Note>
