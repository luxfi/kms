---
title: Token Auth
description: "Learn how to authenticate to KMS from any platform or environment using an access token."
---
...
The following sequence diagram illustrates the Token Auth workflow for authenticating clients with KMS.
...
sequenceDiagram
  ...
  Note over Client,Infis: Access KMS API with Token
...
Token Auth is the simplest authentication method that a client can use to authenticate with KMS.

Unlike other authentication methods where a client must exchange credential(s) for a short-lived access token to access the KMS API,
Token Auth allows a client to make authenticated requests to the KMS API directly using a token. Conceptually, this is similar to using an API Key.

To be more specific:

1. An operator creates an access token in the KMS UI.
2. The operator shares the access token with the client which it can then use to make authenticated requests to the KMS API.
...
<Steps>
  ...
  <Step title="Creating a Token">
    ...
    to an API Key used to authenticate with the KMS API. With that, press **Create Token**.
    ...
    Copy the token and keep it handy as you'll need it to authenticate with the KMS API.
  ...
  <Step title="Accessing the KMS API with the identity">
    To access the KMS API as the identity, you can use the generated access token from step 2
    to authenticate with the [KMS API](/api-reference/overview/introduction).
