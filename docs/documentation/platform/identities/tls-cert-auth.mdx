...
description: "Learn how to authenticate with KMS using TLS Certificate."
---

**TLS Certificate Auth** is an authentication method that verifies a user's TLS Client certificate using the provided CA Certificate, allowing secure access to KMS resources.

## Diagram

The following sequence diagram illustrates the TLS Certificate Auth workflow for authenticating users with KMS.
...
sequenceDiagram
  ...
  Client->>KMS: Send request to /api/v1/auth/tls-cert-auth/login

  Note over KMS: Step 3: Request verification using CA Certificate
...
At a high level, KMS authenticates the client's TLS Certificate by verifying its identity and checking that it meets specific requirements (e.g., it is bound to the allowed common names) at the `/api/v1/auth/tls-cert-auth/login` endpoint. If successful, KMS returns a short-lived access token that can be used to make authenticated requests to the KMS API.

To be more specific:

1. The client sends a TLS request with the client certificate to KMS at the `/api/v1/auth/tls-cert-auth/login` endpoint.
2. KMS verifies the incoming request using the provided CA certificate.
3. KMS checks the user's properties against set criteria such as Allowed Common Names.
4. If all checks pass, KMS returns a short-lived access token that the client can use to make authenticated requests to the KMS API.

<Accordion title="TLS with Load Balancer/Proxy">
  Most of the time, the KMS server will be behind a load balancer or
...
access the KMS API using request signing.
...
Since the identity has been configured with [Universal Auth](https://lux.network/docs/documentation/platform/identities/universal-auth) by default, you should reconfigure it to use TLS Certificate Auth instead. To do this, click the cog next to **Universal Auth** and then select **Delete** in the options dropdown.
...
To access the KMS API as the identity, you need to send a TLS request to `/api/v1/auth/tls-cert-auth/login` endpoint.

Below is an example of how you can authenticate with KMS using NodeJS.
...
try {
  ...
  const kmsUrl = "https://kms.lux.network:8443"; // or your self-hosted KMS URL
  ...
  const { data } = await axios.post(
    `${kmsUrl}/api/v1/auth/tls-cert-auth/login`,
    {
      identityId,
    },
    {
...
- **NEVER** allow direct connections to KMS for TLS cert auth - this enables header injection attacks
...
<Note>
  KMS will handle the actual certificate validation against the configured
