---
title: JWT Auth
description: "Learn how to authenticate with KMS using JWT-based authentication."
---
...
The following sequence diagram illustrates the JWT Auth workflow for authenticating with KMS.
...
At a high-level, KMS authenticates a client by verifying the JWT and checking that it meets specific requirements (e.g. it is signed by a trusted key) at the `/api/v1/auth/jwt-auth/login` endpoint. If successful, then KMS returns a short-lived access token that can be used to make authenticated requests to the KMS API.

To be more specific:

1. The client requests a JWT from their JWT issuer.
2. The fetched JWT is sent to KMS at the `/api/v1/auth/jwt-auth/login` endpoint.
3. KMS validates the JWT signature using either:
   - Pre-configured public keys (Static configuration)
   - Public keys fetched from a JWKS endpoint (JWKS configuration)
4. KMS verifies that the configured claims match in the token. This includes standard claims like subject, audience, and issuer, as well as any additional custom claims specified in the configuration.
...
<Note>
  For JWKS configuration, KMS needs network-level access to the configured
  JWKS endpoint.
</Note>

## Guide

In the following steps, we explore how to create and use identities to access the KMS API using the JWT authentication method.
...
<Step title="Creating an identity">
    ...
    **Common fields for both configurations**:
    ...
    To access the KMS API as the identity, you will need to obtain a JWT from your JWT issuer that meets the validation requirements configured in step 2.

    Once you have obtained a valid JWT, you can use it to authenticate with KMS at the `/api/v1/auth/jwt-auth/login` endpoint.

    We provide a code example below of how you might use the JWT to authenticate with KMS to gain access to the [KMS API](/api-reference/overview/introduction).

   <Accordion
        title="Sample code for inside an application"
    > 
        The shown example uses Node.js but you can use any other language to authenticate with KMS using your JWT.
    ...
    <Tip>
        We recommend using one of KMS's clients like SDKs or the KMS Agent to authenticate with KMS using JWT Auth as they handle the authentication process for you.
    </Tip>
    ...
    If an identity access token exceeds its max TTL or maximum number of uses, it can no longer authenticate with the KMS API. In this case,
