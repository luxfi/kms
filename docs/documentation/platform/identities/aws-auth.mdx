---
title: AWS Auth
description: "Learn how to authenticate with KMS for EC2 instances, Lambda functions, and other IAM principals."
---

**AWS Auth** is an AWS-native authentication method for IAM principals like EC2 instances or Lambda functions to access KMS.

## Diagram

The following sequence diagram illustrates the AWS Auth workflow for authenticating AWS IAM principals with KMS.
...
sequenceDiagram
  ...
  Note over Client,Infis: Step 5: Access KMS API with Token
  Client->>Infis: Make authenticated requests using the short-lived access token
```

## Concept

At a high-level, KMS authenticates an IAM principal by verifying its identity and checking that it meets specific requirements (e.g. it is an allowed IAM principal ARN) at the `/api/v1/auth/aws-auth/login` endpoint. If successful,
...
2. The client sends the signed query data to KMS including the request method, request body, and request headers at the `/api/v1/auth/aws-auth/login` endpoint.
3. KMS reconstructs the query and sends it to AWS STS API via the [sts:GetCallerIdentity](https://docs.aws.amazon.com/STS/latest/APIReference/API_GetCallerIdentity.html) method for verification and obtains the identity associated with the IAM principal.
4. KMS checks the identity's properties against set criteria such **Allowed Principal ARNs**.
5. If all is well, KMS returns a short-lived access token that the IAM principal can use to make authenticated requests to the KMS API.

<Note>
We recommend using one of KMS's clients like SDKs or the KMS Agent
to authenticate with KMS using AWS Auth as they handle the
authentication process including the signed `GetCallerIdentity` query
construction for you.

Also, note that KMS needs network-level access to send requests to the AWS STS API
...
access the KMS API using the AWS Auth authentication method.
...
<Steps>
    ...
    - Allowed Principal ARNs: A comma-separated list of trusted IAM principal ARNs that are allowed to authenticate with KMS. The values should take one of three forms: `arn:aws:iam::123456789012:user/MyUserName`, `arn:aws:iam::123456789012:role/MyRoleName`, or `arn:aws:iam::123456789012:*`. Using a wildcard in this case allows any IAM principal in the account `123456789012` to authenticate with KMS under the identity.
    - Allowed Account IDs: A comma-separated list of trusted AWS account IDs that are allowed to authenticate with KMS.
    ...
    <Step title="Accessing the KMS API with the identity">
        To access the KMS API as the identity, you need to construct a signed `GetCallerIdentity` query using the [AWS Signature v4 algorithm](https://docs.aws.amazon.com/IAM/latest/UserGuide/create-signed-request.html) and make a request to the `/api/v1/auth/aws-auth/login` endpoint containing the query data
        in exchange for an access token.

        We provide a few code examples below of how you can authenticate with KMS from inside a Lambda function, EC2 instance, etc. and obtain an access token to access the [KMS API](/api-reference/overview/introduction).

        <AccordionGroup>
            <Accordion
                title="Sample code for inside a Lambda function"
            >
                The following query construction is an example of how you can authenticate with KMS from inside a Lambda function.
            ...
            <Accordion title="Sample code for inside an EC2 instance">
                The following query construction is an example of how you can authenticate with KMS from inside a EC2 instance.
            ...
            <Accordion title="Sample code for general query construction">
                ...
                ```bash Request
                curl --location --request POST 'https://kms.lux.network/api/v1/auth/aws-auth/login' \
            ...
            <Accordion title="Sample code for inside an EKS pod">
              The following query construction is an example of how you can authenticate with KMS from inside an EKS pod.
              ...
              const main = async () => {
                try {
                  ...
                  const {
                    data: { accessToken }
                  } = await axios.post<{ accessToken: string }>("https://kms.lux.network/api/v1/auth/aws-auth/login", {
                    ...iamRequest,
                    identityId: "<replace-with-your-identity-id>"
                  });

                  console.log(`KMS Access Token: ${accessToken}`);
                } catch (e) {
        ...
        <Tip>
            We recommend using one of KMS's clients like SDKs or the KMS Agent to authenticate with KMS using AWS Auth as they handle the authentication process including the signed `GetCallerIdentity` query construction for you.
        </Tip>
