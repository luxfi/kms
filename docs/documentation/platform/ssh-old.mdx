...
KMS can be used to issue SSH credentials to clients to provide short-lived, secure SSH access to infrastructure;
...
The following concepts are useful to know when working with KMS SSH:
...
When using KMS SSH to provision client access to a remote host, an operator must create an SSH CA in KMS; a certificate template under it,
specifying policies such as allowed users that can be requested under that template by a client; and configure the host to trust certificates issued by the KMS SSH CA.

When a client needs access to a host, they authenticate with KMS and request an SSH certificate (and optionally key pair)
...
sequenceDiagram
  ...
  participant KMS as KMS (SSH CA)
  participant Host as Remote Host

  Note over Client,Client: Step 1: Client Authentication with KMS
  Client->>KMS: Send credential(s) to authenticate with KMS

  KMS-->>Client: Return access token

  Note over Client,KMS: Step 2: SSH Certificate Request
  Client->>KMS: Make authenticated request for SSH certificate via either /api/v1/ssh/issue or /api/v1/ssh/sign

  KMS-->>Client: Return signed SSH certificate (and optionally key pair)
...
At a high-level, KMS issues a signed SSH certificate to a client that can be used to access a remote host.

To be more specific:

1. The client authenticates with KMS; this can be done using a user or machine identity [authentication method](/documentation/platform/identities/machine-identities) or a user [authentication method](/documentation/platform/identities/user-identities).
2. The client makes an authenticated request for an SSH certificate via either the `/api/v1/ssh/issue` or `/api/v1/ssh/sign` endpoints. Note that if the client wishes to use an existing SSH key pair, it can use the `/api/v1/ssh/sign` endpoint; otherwise, it can use the `/api/v1/ssh/issue` endpoint to have KMS issue a new SSH key pair along with the certificate.
...
## Guide to Configuring KMS SSH

In the following steps, we explore how to configure KMS SSH to start issuing SSH certificates to clients as well as a remote host to trust these certificates
...
<Steps>
  ...
  <Step title="Configuring the remote host to trust the client">
    ...
    At this point, the remote host is configured to trust SSH certificates issued by the KMS SSH CA.

  </Step>
</Steps>

## Guide to Using KMS SSH to Access a Host
...
<Steps>
  <Step title="Authenticate with KMS">
...
```
  ...
  <Step title="Obtain an SSH certificate for the client and load it into the SSH agent">
    Run the `kms ssh issue-credentials` command, specifying the `--addToAgent` flag to automatically load the SSH certificate into the SSH agent.
    ```bash
    kms ssh issue-credentials --certificateTemplateId=<certificate-template-id> --principals=<username> --addToAgent
    ```
    ...
    For fuller documentation on commands and flags supported by the KMS CLI for SSH, refer to the docs [here](/cli/commands/ssh).
...
<AccordionGroup>
  <Accordion title="How can I confirm if the KMS SSH workflow is working?">
    After configuring KMS SSH, you can add the `-vvv` flag as part of the
    ...
    ```bash
    debug1: Offering public key: Added via KMS CLI RSA-CERT SHA256:...
    debug1: Server accepts key: Added via KMS CLI RSA-CERT SHA256:...
    ```
