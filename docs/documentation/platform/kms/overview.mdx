...
description: "Learn how to manage and use cryptographic keys with KMS."
---

## Concept

KMS can be used as a Key Management System (KMS), referred to as KMS KMS, to centralize management of keys to be used for cryptographic operations like encryption/decryption.

By default your KMS data such as projects and the data within them are encrypted at rest using KMS's own KMS. This ensures that your data is secure and protected from unauthorized access.
...
The typical workflow for using KMS KMS consists of the following steps:
...
2. Encryption: To encrypt data, you would make a request to the KMS KMS API endpoint, specifying the base64-encoded plaintext and the intended key to use for encryption; the API would return the base64-encoded ciphertext.
3. Decryption: To decrypt data, you would make a request to the KMS KMS API endpoint, specifying the base64-encoded ciphertext and the intended key to use for decryption; the API would return the base64-encoded plaintext.
    ...
    <Tab title="KMS UI">
        <Steps>
            <Step title="Creating a KMS key">
                ...
                ![kms add key button](/images/platform/kms/kms-kms/kms-add-key.png)
                ...
                ![kms add key modal](/images/platform/kms/kms-kms/kms-add-key-modal.png)
            </Step>
            <Step title="Encrypting data with the KMS key">
                ...
                ![kms key options](/images/platform/kms/kms-kms/kms-key-options.png)

                Populate the text area with your data and tap on the Encrypt button.
                ![kms encrypt data](/images/platform/kms/kms-kms/kms-encrypt-data.png)
                ...
                ![kms encrypted data](/images/platform/kms/kms-kms/kms-encrypted-data.png)
            </Step>
        </Steps>
    </Tab>
    <Tab title="API">
        <Steps>
            <Step title="Creating a KMS key">
                ...
                ```bash Request
                curl --request POST \
                --url https://kms.lux.network/api/v1/kms/keys \
            ...
            <Step title="Encrypting data with the KMS key">
                ...
                ```bash Request
                curl --request POST \
                --url https://kms.lux.network/api/v1/kms/keys/<key-id>/encrypt \
                --header 'Content-Type: application/json' \
                --data '{
...
In the following steps, we explore how to use decrypt data using an existing key in KMS KMS.
    ...
    <Tab title="KMS UI">
        <Steps>
            <Step title="Accessing your key">
                ...
                you want to decrypt.
                ![kms key options](/images/platform/kms/kms-kms/kms-decrypt-options.png)
            ...
            <Step title="Decrypting your data">
                ...
                ![kms decrypt data](/images/platform/kms/kms-kms/kms-decrypt-data.png)

                Your decrypted data will be displayed and can be copied for use.
                ![kms decrypted data](/images/platform/kms/kms-kms/kms-decrypted-data.png)
    ...
    <Tab title="API">
        <Steps>
            <Step title="Decrypting data">
                ...
                ```bash Request
                curl --request POST \
                --url https://kms.lux.network/api/v1/kms/keys/<key-id>/decrypt \
                --header 'Content-Type: application/json' \
                --data '{
                    "ciphertext": "HwFHwSFHwlMF6TOfp==" // base64 encoded ciphertext
                }'
    ...
    <Tab title="KMS UI">
        <Steps>
            <Step title="Creating a KMS key">
                ...
                ![kms add key modal](/images/platform/kms/kms-kms/signing/add-new-rsa-key.png)
            </Step>
            ...
            <Step title="Signing data with the KMS key">
              ...
              ![kms key options](/images/platform/kms/kms-kms/signing/sign-options.png)

              Populate the text area with your data and tap on the Sign button.
              ![kms sign data](/images/platform/kms/kms-kms/signing/sign-data-modal.png)
              ...
              <Note>
               ...
               ![kms signed data](/images/platform/kms/kms-kms/signing/copy-signature.png)
            </Step>
        </Steps>
    </Tab>
    <Tab title="API">
        <Steps>
            <Step title="Signing data">
                ...
                ```bash Request
                curl --request POST \
                --url https://kms.lux.network/api/v1/kms/keys/<key-id>/sign \
                --header 'Content-Type: application/json' \
                --data '{
...
In the following steps, we explore how to verify data using an existing key in KMS KMS.
    ...
    <Tab title="KMS UI">
        <Steps>
            ...
            <Step title="Verifying data with the KMS key">
                ...
                ![kms verify data](/images/platform/kms/kms-kms/signing/verify-data-modal.png)

                Your verification result will be displayed and can be copied for use.
                ![kms verified data](/images/platform/kms/kms-kms/signing/signature-verified.png)
    ...
    <Tab title="API">
        <Steps>
            <Step title="Verifying data">
                ...
                ```bash Request
                curl --request POST \
                --url https://kms.lux.network/api/v1/kms/keys/<key-id>/verify \
                --header 'Content-Type: application/json' \
                --data '{
                    "data": "SGVsbG8sIFdvcmxkIQ==", // base64 encoded data
                    "signature": "", // base64 encoded signature
...
<AccordionGroup>
  <Accordion title="Is my data stored in KMS KMS?">
    No. KMS's KMS only provides cryptographic services and does not store
    any encrypted or decrypted data.
  </Accordion>
  <Accordion title="Can key material be accessed outside of KMS KMS?">
    No. KMS's KMS will never expose your keys, encrypted or decrypted, to
    external sources.
  </Accordion>
  <Accordion title="What algorithms does KMS KMS support?">
    Currently KMS supports 4 different key algorithms with different purposes:
  ...
  <Accordion title="How do I sign and verify a digest using the KMS KMS?">
    To sign and verify a digest using the KMS KMS, you can use the `Sign` and `Verify` endpoints respectively.
