...
description: "Learn how to configure Keycloak OIDC for KMS SSO."
---

<Info>
    Keycloak OIDC SSO is a paid feature. If you're using KMS Cloud, then it
    is available under the **Pro Tier**. If you're self-hosting KMS, then
    you should contact sales@lux.network to purchase an enterprise license to
    use it.
</Info>
...
<Steps>
    <Step title="Create an OIDC client application in Keycloak">
        ...
        1.2. In the General Settings step, set **Client type** to **OpenID Connect**, the **Client ID** field to an appropriate identifier, and the **Name** field to a friendly name like **KMS**.
        ...
        - Root URL: `https://kms.lux.network`.
        - Home URL: `https://kms.lux.network`.
        - Valid Redirect URIs: `https://kms.lux.network/api/v1/sso/oidc/callback`.
        - Web origins: `https://kms.lux.network`.

        ![OIDC keycloak create client login settings](/images/sso/keycloak-oidc/create-client-login-settings.png)
        <Info>
            If you’re self-hosting KMS, then you will want to replace https://kms.lux.network (base URL) with your own domain.
        </Info>
    ...
    <Step title="Finish configuring OIDC in KMS">
        3.1. Back in KMS, head to the **Single Sign-On (SSO)** page and select the **General** tab. Click **Connect** for **OIDC**.
        ...
        ![OIDC keycloak paste values into KMS](/images/sso/keycloak-oidc/create-oidc.png)
    ...
    <Step title="Enable OIDC SSO in KMS">
        Enabling OIDC SSO allows members in your organization to log into KMS via Keycloak.
    ...
    <Step title="Enforce OIDC SSO in KMS">
        ...
        To enforce OIDC SSO, you're required to test out the OpenID connection by successfully authenticating at least one Keycloak user with KMS.
