...
description: "Learn how to configure OIDC for KMS SSO with any OIDC-compliant identity provider"
---

<Info>
  OIDC SSO is a paid feature. If you're using KMS Cloud, then it is
...
You can configure your organization in KMS to have members authenticate with the platform through identity providers via [OpenID Connect](https://openid.net/specs/openid-connect-core-1_0.html).
...
<Steps>
   <Step title="Setup Identity Provider">
      1.1. Register your application with the IdP to obtain a **Client ID** and **Client Secret**. These credentials are used by KMS to authenticate with your IdP.

      1.2. Configure **Redirect URL** to be `https://kms.lux.network/api/v1/sso/oidc/callback`. If you're self-hosting KMS, replace the domain with your own.

      1.3. Configure the scopes needed by KMS (email, profile, openid) and ensure that they are mapped to the ID token claims.
   ...
   <Step title="Finish configuring OIDC in KMS">
      2.1. Back in KMS, head to the **Single Sign-On (SSO)** page and select the **General** tab. Select **Connect** for **OIDC**.
     ...
     <Step title="Enable OIDC SSO in KMS">
      Enabling OIDC SSO allows members in your organization to log into KMS via the configured Identity Provider
    ...
    <Step title="Enforce OIDC SSO in KMS">
      ...
      To enforce OIDC SSO, you're required to test out the OpenID connection by successfully authenticating at least one IdP user with KMS.
