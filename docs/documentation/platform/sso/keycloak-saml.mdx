---
title: "Keycloak SAML"
description: "Learn how to configure Keycloak SAML for KMS SSO."
---
...
<Steps>
   <Step title="Prepare the SAML SSO configuration in KMS">
      In KMS, head to the **Single Sign-On (SSO)** page and select the **General** tab. Click **Connect** for **SAML** under the Connect to an Identity Provider section. Select **Keycloak**, then click **Connect** again.
   ...
   <Step title="Create a SAML client application in Keycloak">
      ...
      <Info>
         You don’t typically need to make a realm dedicated to KMS. We recommend adding KMS as a client to your primary realm.
      </Info>

      In the General Settings step, set **Client type** to **SAML**, the **Client ID** field to `https://kms.lux.network`, and the **Name** field to a friendly name like **KMS**.

      ![SAML keycloak create client general settings](../../../images/sso/keycloak/create-client-general-settings.png)

      <Info>
         If you’re self-hosting KMS, then you will want to replace https://kms.lux.network with your own domain.
      </Info>
   ...
   <Step title="Retrieve Identity Provider (IdP) Information from Keycloak">
      ...
      Also, in the **Keys** tab, locate the RS256 key and copy the certificate to use when finishing configuring Keycloak SAML in KMS.

      ![SAML keycloak realm settings keys](../../../images/sso/keycloak/realm-settings-keys.png)
   </Step>
   <Step title="Finish configuring SAML in KMS">
      Back in KMS, set **IDP URL** and **Certificate** to the items from step 3. Also, set the **Client ID** to the `https://kms.lux.network`.

      Once you've done that, press **Update** to complete the required configuration.

      ![SAML Okta paste values into KMS](../../../images/sso/keycloak/idp-values.png)
   </Step>
   <Step title="Enable SAML SSO in KMS">
      Enabling SAML SSO allows members in your organization to log into KMS via Keycloak.
   ...
   <Step title="Enforce SAML SSO in KMS">
      ...
      To enforce SAML SSO, you're required to test out the SAML connection by successfully authenticating at least one Keycloak user with KMS;
