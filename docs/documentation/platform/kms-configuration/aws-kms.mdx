...
To enhance the security of your KMS projects, you can now encrypt your secrets using an external Key Management Service (KMS).
...
<Tabs>
  ...
  <Steps>
    <Step title="Create the Managing User IAM Role">
      ...
      3. Select **Another AWS Account** and provide the appropriate KMS AWS Account ID: use **381492033652** for the **US region**, and **345594589636** for the **EU region**. This restricts the role to be assumed only by KMS. If you are self-hosting, provide the AWS account number where KMS is hosted.
      4. Optionally, enable **Require external ID** and enter your KMS **project ID** to further enhance security.
    </Step>
    <Step title="Add Required Permissions for the IAM Role">
      Use the following custom policy to grant the minimum permissions required by KMS to integrate with AWS KMS
...
<Steps>
  ...
  <Step title="Provide the inputs for AWS KMS">
    ...
    <ParamField path="IAM Role ARN For Role Assumption" type="string">
        ARN of the AWS role to assume for providing KMS access to the AWS KMS Key (required if Authentication Mode is "AWS Assume Role")
    </ParamField>
    ...
    <ParamField path="AWS KMS Key ID" type="string">
    	Key ID of the AWS KMS Key. If left blank, KMS will generate and use a new AWS KMS Key in the specified region.
...
You now have an AWS KMS Key configured at the organization level. You can assign these AWS KMS keys to existing KMS projects by visiting the 'Project Settings' page.
