...
This guide provides instructions on securing KMS project secrets using AWS CloudHSM. 
...
## Connect KMS to AWS KMS Key

You should now have an AWS KMS that has a custom key store set to AWS CloudHSM. 
To secure project resources, you will need to add this AWS KMS to your KMS organization. To learn how, refer to the documentation [here](./aws-kms).