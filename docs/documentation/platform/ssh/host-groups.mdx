...
KMS SSH lets you configure host groups to organize and manage multiple SSH hosts with shared access configuration.
...
The typical workflow for using KMS SSH with host groups consists of the following steps:
...
3. The administrator registers remote hosts with KMS using the KMS CLI via the `kms ssh add-host` command and assigns them to appropriate host groups either using the `--host-group` flag or by adding them to the host group via UI.
4. User(s) access the remote hosts using the KMS CLI via the `kms ssh connect` command, with access determined by the login mappings defined at both host and host group levels.

## Admin Guide for Configuring Host Groups

In the following steps, we'll walk through how to create and configure Host Groups in KMS SSH, and how to add hosts to these groups.

<Steps>
  <Step title="Create a host group">
    1.1. Navigate to your KMS SSH project and select the **Hosts** tab.
    ...
    in that group under specific login user(s); in the allowed principals, you should select user(s) part of the KMS SSH project that will
    be allowed to login to the remote host as the login user.
    
    For instance, if you add a mapping to a host group with the login user `ec2-user` to some users John and Alice in KMS, then they will be allowed to login to any remote host that is part of the group as `ec2-user` which is a system user that
