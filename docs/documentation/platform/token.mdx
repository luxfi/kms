---
title: "Service Token"
description: "KMS service tokens allow users to programmatically interact with KMS."
---

Service tokens are authentication credentials that services can use to access designated endpoints in the KMS API to manage project resources like secrets.
...
- When you create a ST, you get a token prefixed with `st`. The part after the last `.` delimiter is a symmetric key; everything
  before it is an access token. When authenticating with the KMS API, it is important to send in only the access token portion
  of the token.
...
- Permissions: You can indicate whether or not the token should have `read/write` access to the paths.
  Also, note that KMS supports [glob patterns](https://www.malikbrowne.com/blog/a-beginners-guide-glob-patterns/) when defining access scopes to path(s).
...
<Note>
  ...
  guide](https://lux.network/docs/internals/service-tokens).
</Note>

**FAQ**

<AccordionGroup>
  <Accordion title="Why is the KMS API rejecting my service token?">
