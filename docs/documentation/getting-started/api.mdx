...
KMS's REST API is the most flexible way to read/write secrets for your application.

In this brief, we'll explore how to fetch a secret back from a project on [KMS Cloud](https://kms.lux.network) via the REST API.
...
<Steps>
    ...
    <Step title="Create a Client Secret">
        ...
        and password used to authenticate with the KMS API. With that, press on the key icon on the identity to generate a **Client Secret**
        for it.
    ...
    <Step title="Get an access token for the KMS API">
        ...
        Next, we can use the access token to authenticate with the [KMS API](/api-reference/overview/introduction) to read/write secrets
