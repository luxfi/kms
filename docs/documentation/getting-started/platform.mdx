...
This quickstart provides an overview of functionalities offered by KMS.

## Managing your Organization

When you first make an account with KMS, you also create a new **organization** where you are assigned the `admin` role by default.
...
Note that you can configure your organization in KMS to have members authenticate with the platform via protocols like SAML 2.0 and OpenID Connect.
...
The members page lets you add/remove members to/from a project and provision them access to environments via roles. By default, KMS provides the `admin`, `developer`, and `viewer` roles
which you can assign to members.

![project members](../../images/project-members.png)

That's it for the platform quickstart! — We encourage you to continue exploring the documentation to gain a deeper understanding of the extensive features and functionalities that KMS has to offer.

Next, head back to [Getting Started > Introduction](/documentation/getting-started/overview) to explore ways to fetch secrets from KMS to your apps and infrastructure.
