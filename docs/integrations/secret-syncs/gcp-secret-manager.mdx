...
description: "Learn how to configure a GCP Secret Manager Sync for KMS."
---
...
<Tabs>
    <Tab title="KMS UI">
        ...
        ![Configure Options](/images/secret-syncs/gcp-secret-manager/gcp-secret-manager-options.png)
            ...
            - **Initial Sync Behavior**: Determines how KMS should resolve the initial sync.
                ...
                - **Import Secrets (Prioritize KMS)**: Imports secrets from the destination endpoint before syncing, prioritizing values from KMS over GCP Secret Manager when keys conflict.
                - **Import Secrets (Prioritize GCP Secret Manager)**: Imports secrets from the destination endpoint before syncing, prioritizing values from GCP Secret Manager over KMS when keys conflict.
    ...
    <Tab title="API">
        ...
        ```bash Request
        curl    --request POST \
        --url https://kms.lux.network/api/v1/secret-syncs/gcp-secret-manager \
        --header 'Content-Type: application/json' \
        --data '{
            "destinationConfig": {
                "scope": "global",
                "projectId": "kms-test-playground"
            },
            "name": "my-gcp-sync",
