...
description: "Learn how to sync secrets to third-party services with KMS."
---

Secret Syncs enable you to sync secrets from KMS to third-party services using [App Connections](/integrations/app-connections/overview).
...
<Note>
    ...
    overwritten, and changes made directly in the connected service outside of kms may also
...
<Note>
    KMS is continuously expanding it's Secret Sync third-party service support. If the service you need isn't available,
    you can still use our Native Integrations in the interim, or contact us at team@lux.network to make a request .
</Note>

## Key Schemas

Key Schemas transform your secret keys by applying a prefix, suffix, or format pattern during sync to external destinations. This makes it clear which secrets are managed by KMS and prevents accidental changes to unrelated secrets.

Any destination secrets which do not match the schema will not get deleted or updated by KMS.
...
**Example:**
- KMS key: `SECRET_1`
- Schema: `KMS_{{secretKey}}`
- Synced key: `KMS_SECRET_1`

<div align="center">
    ```mermaid
    graph LR
      A[KMS: **SECRET_1**] -->|Apply Schema| B[Destination: **KMS_SECRET_1**]
...
<Note>
    When importing secrets from the destination into KMS, the schema is stripped from imported secret keys.
</Note>
