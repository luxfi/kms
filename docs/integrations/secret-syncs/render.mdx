---
title: "Render Sync"
description: "Learn how to configure a Render Sync for KMS."
---
...
<Tabs>
  <Tab title="KMS UI">
    ...
    ![Configure Options](/images/secret-syncs/render/render-sync-options.png)
      ...
      - **Initial Sync Behavior**: Determines how KMS should resolve the initial sync.
        ...
        - **Import Secrets (Prioritize KMS)**: Imports secrets from the Render service before syncing, prioritizing values from KMS over Render when keys conflict.
        - **Import Secrets (Prioritize Render)**: Imports secrets from the Render service before syncing, prioritizing values from Render over KMS when keys conflict.
  ...
  <Tab title="API">
    ...
    ```bash Request
    curl --request POST \
      --url https://kms.lux.network/api/v1/secret-syncs/render \
