---
title: "TeamCity Sync"
description: "Learn how to configure a TeamCity Sync for KMS."
---
...
<Tabs>
    <Tab title="KMS UI">
        ...
        ![Configure Options](/images/secret-syncs/teamcity/teamcity-sync-options.png)
            ...
            - **Initial Sync Behavior**: Determines how KMS should resolve the initial sync.
                ...
                - **Import Secrets (Prioritize KMS)**: Imports secrets from the destination endpoint before syncing, prioritizing values from KMS over TeamCity when keys conflict.
                - **Import Secrets (Prioritize TeamCity)**: Imports secrets from the destination endpoint before syncing, prioritizing values from TeamCity over KMS when keys conflict.

                <Note>
                    KMS only syncs secrets from within the target scope; inherited secrets will not be imported.
                </Note>
    ...
    <Tab title="API">
        ...
        ```bash Request
        curl    --request POST \
        --url https://kms.lux.network/api/v1/secret-syncs/teamcity \
