...
description: "Learn how to configure a Azure Key Vault Sync for KMS."
---
...
<Note>
  ...
  for KMS to sync secrets to Azure Key Vault: `secrets/list`, `secrets/get`, `secrets/set`, `secrets/recover`.

  Any role with these permissions would work such as the **Key Vault Secrets Officer** role.
</Note>

<Note>
  Secrets in KMS that contain an underscore (`_`) will be converted to a hyphen (`-`) when synced to Azure Key Vault.
</Note>
...
<Tabs>
    <Tab title="KMS UI">
        ...
        ![Configure Options](/images/secret-syncs/azure-key-vault/vault-options.png)
            ...
            - **Initial Sync Behavior**: Determines how KMS should resolve the initial sync.
                ...
                - **Import Secrets (Prioritize Azure Key Vault)**: Imports secrets from the destination endpoint before syncing, prioritizing values from Secrets Manager over KMS when keys conflict.
    ...
    <Tab title="API">
        ...
        ```bash Request
        curl --request POST \
        --url https://kms.lux.network/api/v1/secret-syncs/azure-key-vault \
