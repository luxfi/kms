---
sidebarTitle: "KMSDynamicSecret CRD"
title: "Using the KMSDynamicSecret CRD"
description: "Learn how to generate dynamic secret leases in KMS and sync them to your Kubernetes cluster."
---

## Overview

The **KMSDynamicSecret** CRD allows you to easily create and manage dynamic secret leases in KMS and automatically sync them to your Kubernetes cluster as native **Kubernetes Secret** resources.
This means any Pod, Deployment, or other Kubernetes resource can make use of dynamic secrets from KMS just like any other K8s secret.

This CRD offers the following features:

- **Generate a dynamic secret lease** in KMS and track its lifecycle.
- **Write** the dynamic secret from KMS to your cluster as native Kubernetes secret.
...
- A [machine identity](/docs/documentation/platform/identities/overview) ready for use in KMS that has permissions to create dynamic secret leases in the project.
- You have already configured a dynamic secret in KMS.
- The operator is installed on to your Kubernetes cluster.

## Configure Dynamic Secret CRD

The example below shows a sample **KMSDynamicSecret** CRD that creates a dynamic secret lease in KMS, and syncs the lease to your Kubernetes cluster.

```yaml dynamic-secret-crd.yaml
apiVersion: secrets.lux.network/v1alpha1
kind: KMSDynamicSecret
metadata:
  name: kmsdynamicsecret
spec:
  hostAPI: https://kms.lux.network/api # Optional, defaults to https://kms.lux.network/api
  ...
  # If set to "Revoke", leases will be revoked when the KMSDynamicSecret CRD is deleted.
  ...
  managedSecretReference:
    ...
    secretNamespace: default # Must be the same namespace as the KMSDynamicSecret CRD.
...
Apply the KMSDynamicSecret CRD to your cluster.

```bash
kubectl apply -f dynamic-secret-crd.yaml
```

After applying the KMSDynamicSecret CRD, you should notice that the dynamic secret lease has been created in KMS and synced to your Kubernetes cluster. You can verify that the lease has been created by doing:
...
### KMSDynamicSecret CRD properties
...
The lease revocation policy defines what the operator should do with the leases created by the operator, when the KMSDynamicSecret CRD is deleted.
...
- `Revoke`: The operator will revoke the leases created by the operator when the KMSDynamicSecret CRD is deleted.
...
<Accordion title="dynamicSecret.projectId">
  The project ID of where the dynamic secret is stored in KMS.
</Accordion>

{" "}

<Accordion title="dynamicSecret.environmentSlug">
  The environment slug of where the dynamic secret is stored in KMS.
</Accordion>

{" "}

<Accordion title="dynamicSecret.secretsPath">
  The path of where the dynamic secret is stored in KMS. The root path is
  `/`.
</Accordion>
...
### Applying the KMSDynamicSecret CRD to your cluster

Once you have configured the `KMSDynamicSecret` CRD with the required fields, you can apply it to your cluster. After applying, you should notice that a lease has been created in KMS and synced to your Kubernetes cluster.
...
Deployments referring to Kubernetes secrets containing KMS dynamic secrets don't automatically reload when the dynamic secret lease expires. This means your deployment may use expired dynamic secrets unless manually redeployed.
To address this, we've added functionality to automatically redeploy your deployment when the associated Kubernetes secret containing your KMS dynamic secret updates.
  ...
  template:
    ...
    spec:
      ...
      - name: nginx
        ...
        envFrom:
        - secretRef:
            name: managed-secret # The name of your managed secret, the same that you're using in your KMSDynamicSecret CRD (spec.managedSecretReference.secretName)
        ports:
        - containerPort: 80
```
</Accordion>
