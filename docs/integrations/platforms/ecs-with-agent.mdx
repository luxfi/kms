...
This guide will go over the steps needed to access secrets stored in KMS from Amazon Elastic Container Service (ECS).

At a high level, the steps involve setting up an ECS task with an [KMS Agent](/kms-agent/overview) as a sidecar container. This sidecar container uses [AWS Auth](/documentation/platform/identities/aws-auth) to authenticate with KMS to fetch secrets/access tokens.
...
This guide primarily focuses on integrating KMS Cloud with Amazon ECS on AWS Fargate and Amazon EFS.
However, the principles and steps can be adapted for use with any instance of KMS (on premise or cloud) and different ECS launch configurations.
...
- Understanding of [KMS Agent](/kms-agent/overview)
...
This feature is important for our demonstration, as it allows us to verify whether the KMS agent is depositing the expected files into the designated file volume and if those files are accessible to the application.
...
## Configure Authentication with KMS

In order for the KMS agent to fetch credentials from KMS, we'll first need to authenticate with KMS. Follow the documentation to configure a machine identity with AWS Auth [here](/documentation/platform/identities/aws-auth).
Take note of the Machine Identity ID as you will be needing this in the preceding steps.

## Clone guide assets repository

To help you quickly deploy the example application, please clone the guide assets from this [Github repository](https://github.com/luxfi/kms-guides.git).
This repository contains assets for all KMS guides. The content for this guide can be found within a sub directory called `aws-ecs-with-agent`.
The guide will assume that `aws-ecs-with-agent` is your working directory going forward.

## Deploy example application

Before we can deploy our full application and its related infrastructure with Terraform, we'll need to first configure our KMS agent.

### Agent configuration overview

The agent config file defines what authentication method will be used when connecting with KMS along with where the fetched secrets/access tokens should be saved to.

Since the KMS agent will be deployed as a sidecar, the agent configuration file will need to be encoded in base64.
...
Inside the `aws-ecs-with-agent` directory, you will find a sample `agent-config.yaml` file. This agent config file will connect with KMS Cloud using AWS Auth and deposit access tokens at path `/kms-agent/access-token` and render secrets to file `/kms-agent/secrets`.
...
templates:
  - template-content: |
      ...
      {{- end }}
      {{- end }}
    destination-path: /kms-agent/secrets
```

#### Secret template

The KMS agent accepts one or more optional templates. If provided, the agent will fetch secrets using the set authentication method and format the fetched secrets according to the given template.
...
In the agent configuration above, the template defined will transform the secrets from KMS project with the ID `202f04d7-e4cb-43d4-a292-e893712d61fc`, in the `dev` environment, and secrets located in the path `/`, into a `KEY=VALUE` format.

<Tip>
  Remember to update the project id, environment slug and secret path to one
  that exists within your KMS project
</Tip>
...
resource "aws_ecs_task_definition" "app" {
  ...
  volume {
    ...
    efs_volume_configuration {
      file_system_id = aws_efs_file_system.kms_efs.id
      root_directory = "/"
    }
  }
}
...
After these values have been set, they will be passed to the KMS agent during startup through environment variables, as configured in the `kms-sidecar` container below.
...
...snip...
  {
    ...
    "environment": [
      {
        "name": "KMS_MACHINE_IDENTITY_ID",
        "value": "${machine_identity_id}"
      },
      {
        "name": "KMS_AGENT_CONFIG_BASE64",
        "value": "${agent_config}"
      }
    ],
    "mountPoints": [
      {
          "containerPath": "/kms-agent",
          "sourceVolume": "kms-efs"
      }
    ]
  }
]
...
In the above container definition, you'll notice that that the KMS agent has a `mountPoints` defined.
This mount point is referencing to the already configured EFS volume as shown below.
`containerPath` is set to `/kms-agent` because that is that the folder we have instructed the agent to deposit the credentials to.
...
The `access-token` file should hold a valid `Bearer` token, which can be used to make HTTP requests to KMS.
