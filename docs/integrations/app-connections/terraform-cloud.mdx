...
description: "Learn how to configure a Terraform Cloud Connection for KMS."
---

KMS supports connecting to Terraform Cloud using a service user.

## Setup Terraform Cloud Connection in KMS
...
<Steps>
    ...
    <Step title="Create the API Token">
        Create the API token to be used by KMS.
        <Note>
    ...
    <Step title="Add Terraform Cloud Connection in KMS">
        <Tabs>
            <Tab title="KMS UI">
            ...
            <Tab title="API">
                ...
                ```bash Request
                curl    --request POST \
                        --url https://kms.lux.network/api/v1/app-connections/terraform-cloud \
