...
description: "Learn how to configure an Azure DevOps Connection for KMS."
---

KMS currently supports three methods for connecting to Azure DevOps, which are OAuth, Azure DevOps Personal Access Token and Client Secrets.

<Accordion title="Azure OAuth on a Self-Hosted Instance">
  Using the Azure DevOps <b>OAuth connection</b> on a self-hosted instance of KMS requires configuring an application in Azure
  ...
  **Prerequisites:**
  ...
  <Steps>
    <Step title="Create an application in Azure">
      ...
      <Tip>
        The domain you defined in the Redirect URI should be equivalent to the `SITE_URL` configured in your KMS instance.
      </Tip>
    ...
    <Step title="Add your application credentials to KMS">
      Obtain the **Application (Client) ID** and **Directory (Tenant) ID** (this will be used later in the KMS connection) in Overview and generate a **Client Secret** in Certificate & secrets for your Azure application.
      ...
      Back in your KMS instance, add two new environment variables for the credentials of your Azure application.
      ...
      Once added, restart your KMS instance and use the Azure Client Secrets connection.
    </Step>
    </Steps>    
...
<Accordion title="Azure DevOps personal access token (PAT)">
    ...
    When using the Azure DevOps <b>Access Token connection</b> you'll need to create a new personal access token (PAT) in order to authenticate KMS with Azure DevOps.
    <Steps>
    ...
    <Step title="Copy the new access token">
        Copy the newly created token as this will be used to authenticate KMS with Azure DevOps.
        ...
        <Tabs>
            ...
            <Steps>
                ...
                <Step title="Grant Access">
                    You will then be redirected to Azure to grant KMS access to your Azure account. Once granted,
                    you will be redirected back to KMS's App Connections page. ![Azure Client Secrets
