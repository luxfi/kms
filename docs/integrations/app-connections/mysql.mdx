...
description: "Learn how to configure a MySQL Connection for KMS."
---

KMS supports connecting to MySQL using a database role.

## Configure a MySQL Role for KMS

<Steps>
    <Step title="Create a Role">
        KMS recommends creating a designated role in your MySQL database for your connection.
        ```SQL
        -- create user role
        CREATE USER 'kms_role'@'%' IDENTIFIED BY 'my-password';
        ```
    </Step>
...
<Tabs>
    ...
    <Tab title="API">
        ...
        ```bash Request
        curl    --request POST \
        --url https://kms.lux.network/api/v1/app-connections/mysql \
