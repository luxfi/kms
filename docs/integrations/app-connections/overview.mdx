...
description: "Learn how to manage and configure third-party app connections with KMS."
---

App Connections enable your organization to integrate KMS with third-party services in a secure and versatile way.

## Concept

App Connections are an organization-level resource used to establish connections with third-party applications
that can be used across KMS projects. Example use cases include syncing secrets, generating dynamic secrets, and more.
...
App Connections require initial setup in both your third-party application and KMS. Follow these steps to establish a secure connection:
...
<Tip>
    Whenever possible, KMS encourages creating a designated service account for your App Connection to limit the scope of permissions based on your use-case.
</Tip>
...
3. <strong>Create App Connection:</strong> Configure the connection in KMS using your generated credentials through either the UI or API.

<Info>
    Some App Connections can only be created via the UI such as connections using OAuth.
</Info>

4. <strong>Utilize the Connection:</strong> Use your App Connection for various features across KMS such as our Secrets Sync by selecting it via the dropdown menu
in the UI or by passing the associated `connectionId` when generating resources via the API.

<Note>
    KMS is continuously expanding its third-party application support. If your desired application isn't listed,
    you can still use previous methods of connecting to it such as our Native Integrations.
</Note>

## Platform Managed Credentials

Some App Connections support the ability to have their credentials managed by KMS. By enabling this option,
KMS will modify the credentials to prevent external use of the configured access entity.