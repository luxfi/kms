...
description: "Learn how to configure a Humanitec Connection for KMS."
---

KMS supports connecting to Humanitec using a service user.

## Setup Humanitec Connection in KMS
...
<Steps>
    ...
    <Step title="Create a Service User">
        Create a new service user. Take into account that the role set here will affect the permissions of the API Token so be sure to set it so the Service User has access permissions to the App you want to integrate to KMS.
    ...
    <Step title="Add Service User to Application">
        Move to the **Applications** tab and add the Service User to the Application you want to sync with KMS.
