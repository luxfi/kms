---
title: "KMS Cloud"
description: "Architecture overview of KMS's US and EU cloud deployments"
---

This document provides an overview of KMS's cloud architecture for our US and EU deployments, detailing the core components and how they interact to provide security and infrastructure services.

## Overview

KMS Cloud operates on AWS infrastructure using containerized services deployed via Amazon ECS (Elastic Container Service). Our US and EU deployments use identical architectural patterns to ensure consistency and reliability across regions.

![KMS Cloud Architecture](/images/self-hosting/reference-architectures/KMS-AWS-ECS-architecture.jpeg)

## Components

A typical KMS Cloud deployment consists of the following components:

### Application Services

- **KMS Core**: Main application server running the KMS backend API
...
The KMS application runs as multiple containerized services on ECS:
...
- License API exposes port 4000 (portal.lux.network, license.lux.network)
...
4. **Application logic** processes requests in the KMS Core service
...
### US Cloud (us.lux.network or kms.lux.network)
...
- **Images**: `kms/staging_kms` and `kms/license-api` from Docker Hub
