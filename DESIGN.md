...
2. **RT keygen**: The MPC daemon runs distributed ML-DSA-65 key generation. Each node holds a share. The combined public key and proof of possession follow the same pattern as BLS using the Corona threshold package.
...
- Add RT threshold DKG to MPC daemon (using `luxfi/Corona/threshold`).
