  ...
  const handleSamlGroupManagement = async (value: boolean) => {
    ...
    if (!subscription?.samlSSO || !subscription?.groups) {
      handlePopUpOpen("upgradePlan", {
        ...
        text: "Your current plan does not include access to SAML group mapping. To unlock this feature, please upgrade to KMS Enterprise plan."
      });
      return;
    }
    ...
    <div className="space-y-4">
      ...
      {data && GROUP_SYNC_SUPPORTED_PROVIDERS.includes(data.authProvider) && (
        <div className="py-4">
                ...
                content={
                  <>
                    <p>
                      ...
                      memberships between the SAML provider and KMS. Users will be added to
                      KMS groups that match their SAML group names.
                    </p>
                    <p className="mt-4">
                    ...
                    <a
                      ...
                      href="/docs/documentation/platform/sso/overview"
                    >
                      See your SAML provider docs for details.
                    </a>
                    <p className="mt-4 text-yellow">
                      <FontAwesomeIcon className="mr-1" icon={faWarning} />
                      Group membership changes in the SAML provider only sync with KMS when a
                      user logs in via SAML. For example, if you remove a user from a group in the
                      SAML provider, this change will not be reflected in KMS until their next
                      SAML login. To ensure this behavior, KMS recommends enabling Enforce
                      SAML SSO.
                    </p>
                  </>
                }
          ...
          <p className="text-sm text-mineshaft-300">
            KMS will manage user group memberships based on the SAML provider
          </p>
        </div>
      )}
      <SSOModal
